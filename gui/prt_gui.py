...
#   The 2D release is three Jupyter notebooks. Running it means opening the
#   right notebook, finding the cell that holds the conditions, editing numbers
#   in place and running every cell below. That is fine for three reactions and
#   one person, and it does not survive a campaign of thousands of runs across
#   a cluster, which is what the 3D work is.
#
#   This window is not a replacement for a command line and must never become
#   one. It BUILDS a command line, shows it in a box that can be copied, and
#   runs it. Everything it can do, prt.py can do with the same flags and the
#   same defaults, which is what makes a job that was set up here runnable on
#   GACRC where there is no display. That is checked rather than asserted:
#   test_gui_commands.py takes every button and requires the script it calls to
#   accept the flags it sends.
#
#   The other change is the flow-aware path, which is a SEQUENCE and not a
#   checkbox. It has its own page, because presenting five ordered steps as a
#   tick box on the training page invited people to tick it and get the last
#   step without the first four.
#
...
def _dep_ok(fields, dep):
    ...
    A dependency is normally one (key, wanted value) pair. It may also be a
    LIST of such pairs, in which case every one of them has to hold. That is
    what lets the lowest-Damkohler box be shown only when the biotic reaction
    is on AND the user has asked for a range rather than exact numbers,
    without inventing a second box for the second condition.
    """
    if not dep:
        return True
    if (isinstance(dep, (tuple, list)) and dep
            and all(isinstance(d, (tuple, list)) for d in dep)):
        return all(_dep_ok(fields, d) for d in dep)
...
TREE_HINTS = {
    ...
    "collect_foreign":     "for runs you set up by hand, outside this program",
...
def physical_fields():
    ...
    return [
        ...
        # The extra ("da_mode", "a range") condition matters only on pages that
        # HAVE a da_mode box. _dep_ok treats a missing guard as satisfied, so
        # every other page behaves exactly as it did before.
        Field("da_min", "Lowest BIOTIC Damkohler", "float", 0.1,
              ...
              "the problem is interesting.", flag="--da-min",
              depends=[BIO, ("da_mode", "a range")]),
        Field("da_max", "Highest BIOTIC Damkohler", "float", 10.0, "",
              flag="--da-max", depends=[BIO, ("da_mode", "a range")]),
...
# =============================================================================
def _promote_settings_box(fields):
    """Move the "load a settings file" box to the top of the page.

    It used to sit at position 60 of 68, under seven sections of physics and a
    heading about saving files, which is the same as not being there: the first
    person who wanted to import an XML could not find it. Loading a file is the
    FIRST thing you do on a page, before you touch any of the boxes it fills
    in, so that is where it belongs.

    The box that SAVES a settings file stays at the bottom, because saving is
    the last thing you do. Only the loader moves, and only on pages that have
    one -- every other page is returned unchanged.
    """
    i = next((n for n, f in enumerate(fields) if f.key == "settings"
              and f.kind == "file"), None)
    if i is None or i <= 2:
        return fields
    box = fields.pop(i)
    box.label = "Load everything from a settings file  (.xml or .json)"
    box.help = ("Optional, and the fastest way to set up a run: one file holds "
                "the grid, the flow, the chemistry, and which Peclet numbers, "
                "Damkohler numbers and porosities to use. Point at it here and "
                "every box below is filled from it. Anything you then type in "
                "a box wins over the file, so you can load a colleague's "
                "settings and change one number without editing their file. "
                "A real CompLaB.xml works here too. There is a commented "
                "example in examples/example_settings_3d.xml, and the box at "
                "the bottom of this page writes one out for you.")
    head = Field("HD_load", "Start from a settings file, or fill the boxes in",
                 "heading", "",
                 "Skip this and type the numbers in yourself; nothing here is "
                 "required.")
    # After the output path, which is the one thing a run cannot do without.
    at = 1 if fields and fields[0].kind in ("savefile", "file", "dir") else 0
    fields[at:at] = [head, box]
    # The heading the box used to live under now covers only the two boxes that
    # are still there, so it should stop promising a loader.
    for f in fields:
        if f.kind == "heading" and f.label.startswith("Settings files,"):
            f.label = "Saving these numbers, and checking before you run"
            f.help = ("The boxes above already hold every physical input. "
                      "These are for writing them out so they can be reused, "
                      "changing one of them without editing a file, and "
                      "reading everything back before you spend hours on it.")
    return fields
...
def build_actions():
    ...
    A["dataset3d"] = Action(
        ...
        [Field("out", "Save the dataset as", "savefile",
         ...
         Field("n_sets", "How many conditions to run on each rock", "int", 5,
               "One simulation per condition. A condition is one combination "
               "of Peclet and Damkohler. Leave this blank when you have typed "
               "exact numbers below and it is filled in for you: three Peclet "
               "numbers and two Damkohler numbers is six conditions.",
               flag="--n-sets", optional=True),
         Field("SIMCOUNT", "", "computed", "", ""),
         Field("n_times", "Snapshots in time", "int", 8, "", flag="--n-times"),

         Field("phi_mode", "Porosity", "choice", "a range",
               "A RANGE gives every rock its own porosity, drawn at random "
               "inside it, which is what you want for a training set that has "
               "to cover the range. EXACT NUMBERS builds rocks at the "
               "porosities you name and nothing else, which is what you want "
               "when you are reproducing published conditions or comparing "
               "like with like.",
               choices=["a range", "exact numbers"]),
         Field("phi_min", "Lowest porosity", "float", 0.30,
               ...
               flag="--phi-min", depends=("phi_mode", "a range")),
         Field("phi_max", "Highest porosity", "float", 0.50, "",
               flag="--phi-max", depends=("phi_mode", "a range")),
         Field("phi_values", "The porosities to build", "text", "0.35 0.40 0.45",
               "Separate them with spaces. Rocks cycle through the list in "
               "order, so twelve rocks over three porosities gives four of "
               "each. Below about 0.20 the pore space stops spanning the "
               "sample in 3D and the generator will reject nearly everything "
               "it makes.",
               flag="--phi-values", nargs="*",
               depends=("phi_mode", "exact numbers")),

         Field("pe_mode", "Peclet number", "choice", "a range",
               "A RANGE draws a fresh Peclet number for every run, spaced "
               "logarithmically, which spreads the runs over the decades "
               "rather than crowding them at the top. EXACT NUMBERS uses the "
         ...
         Field("pe_min", "Lowest Peclet", "float", 1.0,
               ...
               "per run than the advective end.",
               flag="--pe-min", depends=("pe_mode", "a range")),
         Field("pe_max", "Highest Peclet", "float", 50.0, "",
               flag="--pe-max", depends=("pe_mode", "a range")),
         Field("pe_values", "The Peclet numbers to run", "text", "1 10 50",
               "Separate them with spaces. Every rock is run at every one of "
               "them, crossed with the Damkohler numbers below.",
               flag="--pe-values", nargs="*",
               depends=("pe_mode", "exact numbers")),

         Field("da_mode", "Damkohler number", "choice", "a range",
               "The same choice again, for the reaction rate relative to the "
               "rate at which the flow brings fresh reactant in. The two "
               "Damkohler boxes further down, under the reaction settings, "
               "are the range this refers to.",
               choices=["a range", "exact numbers"]),
         Field("da_values", "The biotic Damkohler numbers to run", "text",
               "0.5 5",
               "Separate them with spaces. Every rock is run at every "
               "combination of these and the Peclet numbers above.",
               flag="--da-values", nargs="*",
               depends=[("ph_mode", ("biotic only", "both")),
                        ("da_mode", "exact numbers")]),
         Field("da_abio_values", "The abiotic Damkohler numbers to run", "text",
               "",
               "Optional, and only if the abiotic reaction is on. Leave it "
               "blank to keep drawing them from the range below.",
               flag="--da-abio-values", nargs="*", optional=True,
               depends=[("ph_mode", ("abiotic only", "both")),
                        ("da_mode", "exact numbers")]),
    ...
    A["heewon"] = Action(
        ...
        [Field("checkpoint", "Their trained model", "file",
         ...
         Field("grid", "The grid it was trained on (nx ny)", "text", "148 64",
               ...
               flag="--grid", nargs=2),
         Field("n_params", "Parameters in YOUR dataset", "int", 2,
               "Theirs takes 2, Pe and Da, and so does ours by default, so at 2 "
               "every weight loads. Raise it only if your dataset carries a "
               "third dimensionless group, and the first layer of their "
               "parameter branch is then skipped rather than silently "
               "reshaped.", flag="--n-params"),
         Field("species", "Which chemical this warm start is for", "text", "C",
               "The network predicts one field, so one warm start belongs to "
               "one chemical. This name is recorded in the saved checkpoint.",
               flag="--species"),
        ...
        ["Maps their layer names onto ours and reports what transferred, what "
         ...
         "Their bias loads too: they predict one field and so do we, so with "
         "two parameters every tensor in their checkpoint has a counterpart of "
         "the same shape here and the load is exact.",
    ...
    A["train"] = Action(
        ...
        [Field("data", "Dataset", "file", "",
         ...
         Field("patience", "Stop after this many bad epochs", "int", 20, "",
               flag="--patience"),
         Field("species", "Which chemical to predict", "text", "",
               "The network predicts ONE concentration field, exactly as the "
               "published 2D model does, so a dataset with four chemicals needs "
               "four training runs. Type the name as the dataset spells it: Ac, "
               "A, P or Bio. Leave it blank and the first chemical in the file "
               "is used, and the run prints which one that was.",
               flag="--species", optional=True),
         # The chemistry and the distance convention. Neither changes a tensor
         # shape, so neither can be caught by anything downstream: they are here
         # because the checkpoint has to record what the model is FOR, or a
         # model trained on one chemistry is later used to predict another with
         # every shape agreeing and the field merely wrong.
         Field("reaction", "Which chemistry this model is for", "choice",
               "acetate_sulfate",
               "The chemicals and the dimensionless numbers the model is being "
               "fitted to. It is written into the checkpoint, so 'Evaluate' and "
               "'Predict' can say when a model is being used on the wrong "
               "chemistry. The three published ones are there so the released "
               "2D weights can be warm-started under the right settings.",
               choices=["acetate_sulfate", "aom_sulfate", "monod",
                        "irreversible_sorption", "reversible_sorption"],
               flag="--reaction"),
         Field("distance_convention", "   how the distance is scaled", "choice",
               "ours",
               "'ours' is zero at the inlet and rises downstream, which is what "
               "our own dataset builders store. The published two are ONE at the "
               "inlet and fall, so they run the opposite way. Leave this alone "
               "unless warm-starting from the released 2D weights.",
               choices=["ours", "published", "published_pore"],
               flag="--distance-convention"),
         ...
         Field("SW_C", "SWITCH C — dimension-free coordinates", "bool", False,
               ...
               flag="--dim-free"),
         # AUDIT GUI-02. Switch D used to be two boxes here. It is not one
         # setting: it needs flow descriptors in the dataset, a velocity model
         # trained, and a predicted field written back, in that order, and a
         # box cannot say so. It moved to the flow pipeline panel, which runs
         # the five steps in order and stops when one fails. The flag still
         # reaches train.py, through the two pipeline actions below.
        ...
        [("best.pt",
          "the trained network for the chemical you chose. This is the thing "
          "that replaces the simulation."),
         ("summary.json",
          "the final held-out error, plus the full training history")],
        group="Learn",
    ...
    A["sweep"] = Action(
        ...
        [Field("data", "3D dataset", "file", "", "", flag="--data",
         ...
         Field("species", "Which chemical to predict", "text", "",
               "Every configuration is trained on the same chemical, or the "
               "comparison means nothing. Blank uses the first one in the file.",
               flag="--species", optional=True),
         Field("quick", "Quick plumbing check only", "bool", False,
    ...
    # Every page that can load a settings file offers it at the top rather than
    # at the bottom. Done here, once, so no page can be built having forgotten.
    for _act in A.values():
        _act.fields = _promote_settings_box(list(_act.fields))
        for _f in _act.fields:
            _f.siblings = _act.fields

    # ------------------------------------------------- 3c. FOREIGN COLLECT
    # The campaign builder writes two small files into every run folder saying
    # what conditions it used and whether it finished. A run set up by hand has
    # neither, and a .vti file cannot supply them: it holds an array of numbers
    # on a grid and nothing else. This page is that case.
    A["collect_foreign"] = Action(
        "collect_foreign", "Collect CompLaB runs you set up by hand",
        "For output that did not come from the campaign builder on this "
        "machine, so it carries no record of its own conditions.",
        os.path.join(TOOLS, "collect_foreign_complab.py"),
        [Field("runs", "Folder holding the runs", "dir", "",
               "One subfolder per run. Each must hold its geometry file and "
               "its .vti output. They do not need to be named run_0000.",
               flag="--runs"),
         Field("out", "Folder to write the dataset into", "dir",
               os.path.join(NEW3D),
               "dataset.h5 is written inside it.", flag="--out"),
         Field("inspect", "Only report what is there, write nothing", "bool", True,
               "Do this first. It says, run by run, which code it decided "
               "means pore, which conditions it found and where, and which it "
               "could not find. One round trip now is cheaper than a dataset "
               "built on a wrong assumption.",
               flag="--inspect"),

         Field("geometry_file", "The geometry file inside each run", "text",
               "inputGeom.vti",
               "The file holding the pore structure. CompLaB writes "
               "inputGeom.vti.", flag="--geometry-file"),
         Field("species", "Chemicals, as name=pattern", "text",
               "C=subsLattice0_*.vti",
               "One entry per chemical, separated by spaces. The name is what "
               "it will be called in the dataset, the pattern is how its files "
               "are named. For a four chemical run: Ac=Ac_*.vti A=A_*.vti "
               "P=P_*.vti Bio=Bio_*.vti",
               flag="--species", nargs="*"),
         Field("flow_glob", "The flow field file", "text", "nsLattice*.vti",
               "It looks inside for a three component array first, and falls "
               "back to the magnitude only if there is no vector. A magnitude "
               "makes the flow look purely axial everywhere, so travel time "
               "computed from it is fiction, and the run is flagged.",
               flag="--flow-glob"),
         Field("no_velocity", "Skip the flow field", "bool", False,
               "Leave this off. Without the flow field the flow proxy and "
               "dimension free options cannot be used at all.",
               flag="--no-velocity"),

         Field("pore_code", "Which value means pore", "int", "",
               "Leave blank and it is worked out by matching each code's share "
               "of the box against the porosity in your conditions file. Fill "
               "it in if there is no porosity to match against. THIS MATTERS: "
               "CompLaB's codes are set per run, so a file written with 0 = "
               "pore fed in as 0 = solid trains the network on the rock "
               "instead of the pore space, and nothing complains.",
               flag="--pore-code", optional=True),

         Field("conditions_csv", "Also read a summary csv inside each run",
               "text", "",
               "Leave this BLANK. The conditions are taken from the run's "
               "CompLaB.xml, which is the file the solver actually read. A "
               "summary csv is written by whatever script happened to "
               "post-process the run, differs between campaigns, and a number "
               "lifted out of one is indistinguishable afterwards from a "
               "number the solver was given. Name one here only if you want it "
               "read as well, and it will fill gaps rather than override the "
               "input file.",
               flag="--conditions-csv", optional=True),
         Field("pe_column", "Its Peclet column", "text", "Pe_achieved",
               "Which column of that csv holds the Peclet number.",
               flag="--pe-column"),
         Field("porosity_column", "Its porosity column", "text", "Porosity",
               "Used to work out which code means pore.",
               flag="--porosity-column"),

         Field("pe", "Peclet", "float", "",
               "Type it in if there is no conditions file, or to override one. "
               "Applies to every run in the folder.",
               flag="--pe", optional=True),
         Field("da_bio", "Damkohler, microbial reaction", "float", "",
               "Leave blank for a run with no microbial reaction. Blank is "
               "recorded as zero and the report says it was not found.",
               flag="--da-bio", optional=True),
         Field("da_abio", "Damkohler, surface reaction", "float", "",
               "This one can never be read back from CompLaB.xml, because "
               "CompLaB takes it from an environment variable rather than the "
               "input file. If the runs had a surface reaction, it has to be "
               "typed here.", flag="--da-abio", optional=True),
         Field("ks_ac", "Half saturation, donor", "float", "",
               "Divided by its inlet concentration.",
               flag="--ks-ac", optional=True),
         Field("ks_a", "Half saturation, acceptor", "float", "",
               "Divided by its inlet concentration.",
               flag="--ks-a", optional=True),
         Field("y", "Yield", "float", "",
               "Times the donor inlet concentration, over the initial biomass.",
               flag="--y", optional=True),

         Field("max_conc", "Reject a run above this concentration", "float", 1.0,
               "The blow up check. Concentrations are expected to be "
               "normalised to the inlet, so anything much above 1 means the "
               "run diverged, or that yours are not normalised.",
               flag="--max-conc"),
         Field("clip", "Clip to that bound instead of rejecting", "bool", False,
               "Only once you know why the run goes above it. Clipping a "
               "diverged run hides the divergence.", flag="--clip"),
         Field("no_provenance",
               "Do not copy the conditions file into the dataset", "bool", True,
               "By default every column of the conditions file is kept "
               "alongside the data, so the dataset can always say what made "
               "it. Tick this to leave it out and keep only the run names. "
               "The file is still READ for the Peclet number and for working "
               "out which code means pore.", flag="--no-provenance"),
         Field("xml", "Where the CompLaB.xml files are  (blank is normal)",
               "file", "",
               "LEAVE THIS BLANK unless the input files are somewhere the "
               "search will not reach. Blank means every run gets ITS OWN "
               "input file: each run folder is searched, then an input "
               "subfolder inside it, then the folder holding all the runs. "
               "That is how a campaign is normally laid out and it is the "
               "answer you want. Name one file here and that same file is used "
               "for every run, which is wrong if the runs differ. You can also "
               "type a FOLDER path: a subfolder in it named after the run is "
               "used first, then one file sitting directly in it. "
               "Optional. CompLaB reads its input file from the folder it was "
               "launched in and never copies it next to the results, so old "
               "output usually arrives with nothing describing it. Point at "
               "the file, or at a folder holding it, and the whole input file "
               "is kept inside the dataset: the text word for word, and every "
               "setting as a readable table. Leave it blank and each run "
               "folder and its parent are searched. A run with no input file "
               "is still collected. The dataset simply records that there was "
               "none, and where it looked.",
               filetypes=[("CompLaB input", "*.xml"), ("All files", "*.*")],
               flag="--xml", optional=True),
         Field("kinetics",
               "Where the defineKinetics.hh files are  (blank is normal)",
               "file", "",
               "Found the same way as the input file, and blank is again the "
               "normal answer. The rate constants live in this file rather "
               "than in the input file, so without it a result cannot be "
               "reproduced. The whole file is kept, and its constants are "
               "pulled out with the note beside each one so the chemistry can "
               "be read without opening it.",
               filetypes=[("Kinetics header", "*.hh"), ("All files", "*.*")],
               flag="--kinetics", optional=True),
         Field("abiotic_kinetics",
               "Where the defineAbioticKinetics.hh files are  (blank is normal)",
               "file", "",
               "Same search again. The purely chemical reactions, the ones "
               "with no microbe involved, plus the mineral dissolution rate. "
               "There is no such file in the 2D version, and a run without one "
               "collects normally.",
               filetypes=[("Kinetics header", "*.hh"), ("All files", "*.*")],
               flag="--abiotic-kinetics", optional=True),
         Field("no_inputs", "Do not look for the input files at all",
               "bool", False,
               "Skips the search entirely. Everything else is collected "
               "exactly as before.", flag="--no-inputs"),

         Field("rate_glob", "The reaction rate files inside each run", "text",
               "rateLattice*.vti",
               "Written only when <save_reaction_rates> was on in the input "
               "file. Every array inside becomes one channel, named as the "
               "kinetics header named it, so nothing here has to know the "
               "chemistry. A run without them collects exactly as before.",
               flag="--rate-glob"),
         Field("no_rates", "Do not look for the reaction rate files",
               "bool", False,
               "Use this if the rate files are large and you only want the "
               "concentrations.", flag="--no-rates")],

        [("the run folders", "one per run, holding the geometry file, the "
          ".vti output and, if there is one, the csv the run wrote about "
          "itself"),
         ("the conditions", "from that csv where it has them, from the boxes "
          "above otherwise. Nothing is invented: whatever cannot be found is "
          "named in the report and recorded as zero")],

        ["Works out which code in the geometry file means pore, by matching "
         "each code's share of the box against the porosity you supplied, and "
         "PRINTS the decision. Then translates the codes to ours, finding the "
         "wall as whatever actually touches the pore space rather than "
         "assuming it.",
         "Computes the two distance fields from that structure, since a hand "
         "run has neither.",
         "Reads every .vti, keeping the iteration number out of each filename "
         "so that time means the same thing across runs of different length.",
         "Stores one entry per DISTINCT rock. Runs that share a structure "
         "share it in the file, and each run records which one it used.",
         "Checks the fields and rejects a run that diverged, came out empty or "
         "went above the bound, naming the reason.",
         "Keeps every column of the conditions csv alongside the data, so the "
         "file can always say what made it.",
         "Keeps the input file and the kinetics file each run was given, "
         "whole, plus every setting as a readable table. Runs that shared one "
         "input file store it once. A run that had none is recorded as having "
         "none, with the places that were searched, rather than being "
         "rejected or filled in with plausible defaults.",
         "Nothing missing stops a run being collected. No input file, no "
         "kinetics file, no conditions csv, no flow field, a chemical the run "
         "never wrote, fewer snapshots than its neighbours: each is recorded "
         "and the rest of the run is kept."],

        [("dataset.h5", "the same file the normal collect page writes, and "
          "read by exactly the same training, evaluation and viewer pages")],
        group="Simulate")


    # ------------------------------------------------------------------ AUDIT GUI-02
    # The two training runs the flow pipeline makes. They are separate actions
    # rather than one action with a box, because the whole point of the
    # pipeline is that the control run happens FIRST and decides whether the
    # rest is worth doing. Two actions also means two output folders, so the
    # second cannot quietly overwrite the first.
    def _flow_train(key, title, mode, one_line, why):
        return Action(
            key, title, one_line, os.path.join(MODEL, "train.py"),
            [Field("data", "Dataset", "file", "", "the .h5 to train on",
                   flag="--data", filetypes=[("HDF5 dataset", "*.h5")]),
             Field("out", "Save the run in", "dir", "", "", flag="--out"),
             Field("velocity_informed", "", "choice", mode, "",
                   flag="--velocity-informed", choices=[mode]),
             Field("geom_features", "Also show it the pore size maps", "bool",
                   False, "MIS and UPRM. The dataset must carry them; step 1 "
                   "puts them into one that does not.", flag="--geom-features"),
             Field("epochs", "Epochs", "int", 300, "", flag="--epochs"),
             Field("workers", "Loader processes", "int", 4, "", flag="--workers")],
            [("dataset.h5", "the training data"),
             ("the velocity field", "simulated, or predicted by step 4")],
            [why],
            [("best.pt", "the trained network"),
             ("summary.json", "the held-out score")],
            group="Learn",
            progress_re=r"^epoch\s+(\d+)", total_re="epochs",
            metric_re=r"^epoch\s+(\d+)\s+train\s+([\d.eE+-]+)\s+val\s+([\d.eE+-]+)")

    A["train_flow_sim"] = _flow_train(
        "train_flow_sim", "Train on the SIMULATED flow field", "simulated",
        "Switch D with the flow the solver already produced.",
        "Trains with the velocity field the dataset already holds. No velocity "
        "model is needed, and the score it reaches is the best a predicted "
        "field could ever match. This is the control the rest is measured "
        "against, which is why it comes before the velocity operator.")

    A["train_flow_pred"] = _flow_train(
        "train_flow_pred", "Train on the PREDICTED flow field", "predicted",
        "Switch D with a field the velocity model wrote.",
        "Trains with the velocity field step 4 wrote back into the dataset. "
        "This is the full two-stage pipeline, and the number to compare "
        "against the control run from step 2.")

    return A


# =============================================================================
#  THE FLOW PIPELINE — five steps, in order, with the wiring done for you
# =============================================================================
# AUDIT GUI-02. gui/test_flow_panel.py described this panel in detail and the
# panel did not exist, so the test failed on import and the flow capability had
# no guided path through the GUI at all.
#
# Why a panel and not five buttons. Each of the five commands is valid on its
# own, and argparse cannot see the mistake that actually costs an afternoon:
# step 4 writing to one file while step 5 reads another, or step 5 running at
# all after step 4 failed. The panel holds one dataset, one output folder and
# one epoch count, fills every command from them, and stops the moment a step
# returns non-zero.
#
#   key           what it does                                action
#   descriptors   add MIS and UPRM to the dataset             flow_features
#   control       train on the SIMULATED field                train_flow_sim
#   operator      train the velocity model                    train_velocity
#   predict       predict the field, write it back            predict_velocity
#   flowmodel     train on the PREDICTED field                train_flow_pred
#
# The control run is second on purpose: it costs one training run and tells you
# whether the velocity operator is worth training at all.
FLOW_STEPS = [
    ("descriptors", "1. Add the pore size maps",
     "MIS and UPRM go into the dataset. Nothing is trained.",
     "Needed by every later step that asks for the pore size maps.",
     "flow_features"),
    ("control", "2. Train on the simulated flow field",
     "Switch D, using the field the solver already produced.",
     "The control. It is the best a predicted field could do, and it costs one "
     "training run to find out.",
     "train_flow_sim"),
    ("operator", "3. Train the velocity model",
     "Learns to produce a flow field from the pore structure alone.",
     "Only worth doing if step 2 beat the plain model.",
     "train_velocity"),
    ("predict", "4. Predict the flow field and write it back",
     "Runs the velocity model over every rock and stores the result.",
     "Writes samples/velocity_pred into the dataset, which is what step 5 reads.",
     "predict_velocity"),
    ("flowmodel", "5. Train on the predicted flow field",
     "Switch D again, this time on the field step 4 wrote.",
     "The full pipeline. Compare its score with step 2.",
     "train_flow_pred"),
]


class FlowPipelinePage(tk.Frame):
    """The five steps of the flow capability, wired to each other.

    The panel owns the settings the steps share, so they cannot disagree:
    one dataset, one output folder, one buffer, one conditioning variable and
    one epoch count. Each step gets its own subfolder under the output folder,
    so the two training runs can be compared afterwards instead of one
    replacing the other.
    """

    def __init__(self, parent, app):
        # __new__ is used by the test, so nothing here may depend on tk having
        # built a real widget. Every attribute the page needs is set before any
        # widget is touched.
        self.app = app
        self.queue = []                 # step keys still to run
        self.running = None             # the step key running now, or None
        ...
        self.cards = {}
        for key, title, _sub, _why, _act in FLOW_STEPS:
            self.cards[key] = {"do": tk.BooleanVar(value=True),
                               "title": title, "state": None}
        try:
            super().__init__(parent, bg=BG)
            self._build()
        except Exception:
            # Built headless by the test: the wiring above is what it checks.
            pass

    # ----------------------------------------------------------- the commands
    def _out_for(self, key):
        """Each step writes into its own folder under the shared one."""
        base = self.v_out.get()
        return os.path.join(base, {"control": "train_simulated",
                                   "operator": "velocity_operator",
                                   "predict": "velocity_prediction",
                                   "flowmodel": "train_predicted"}.get(key, key))

    def _fill(self, act_key):
        """The action, and the exact command line for it.

        Every value comes from this panel, so no two steps can disagree about
        the dataset, the buffer or the epoch count.
        """
        act = self.app.actions[act_key]
        data = self.v_data.get()
        for f in act.fields:
            # A field only grows its variable when its own page is drawn, and
            # the pipeline fills actions whose pages may never have been
            # opened. Make it here rather than reaching into a None.
            if f.var is None:
                f.make_var()
            if f.key == "data":
                f.var.set(data)
            elif f.key == "buffer":
                f.var.set(self.v_buffer.get())
            elif f.key == "condition":
                f.var.set(self.v_condition.get())
            elif f.key == "epochs":
                f.var.set(self.v_epochs.get())
            elif f.key == "geom_features":
                f.var.set(bool(self.v_geom.get()))
            elif f.key == "out":
                f.var.set(self._out_for(
                    {"train_flow_sim": "control",
                     "train_velocity": "operator",
                     "predict_velocity": "predict",
                     "train_flow_pred": "flowmodel"}.get(act_key, act_key)))
            elif f.key == "checkpoint" and act_key == "predict_velocity":
                # step 4 reads exactly what step 3 wrote
                f.var.set(os.path.join(self._out_for("operator"), "best.pt"))
            elif f.key == "write_back" and act_key == "predict_velocity":
                f.var.set(True)
            elif f.key in ("geometry", "pe", "condition_value"):
                # Predicting for a whole dataset and naming one flow condition
                # are different requests. Sending both contradicts the file's
                # own recorded condition with a number left in a box.
                f.var.set("" if f.kind != "bool" else False)
        return act, act.command()

    # ------------------------------------------------------------- the running
    def _start(self, key):
        act, cmd = self._fill(_action_of(key))
        if not os.path.exists(act.script):
            self.app.logline("step %s: %s is not in this checkout, stopped here"
                             % (key, act.script), "bad")
            self.queue = []
            self.running = None
            return
        self.running = key
        self.cards[key]["state"] = "running"
        self.app.on_run_finished = self._finished
        self.app.launch(act, cmd, None)

    def _finished(self, rc):
        key, self.running = self.running, None
        if key is not None:
            self.cards[key]["state"] = "ok" if rc == 0 else "failed"
        if rc != 0:
            # Carrying on would run the next step against something the failed
            # one never wrote, and the error the user reads would be a missing
            # key rather than the real failure minutes earlier.
            self.app.logline(
                "step %s failed with code %s, so the pipeline stopped here. "
                "The steps after it read what it was supposed to write."
                % (key, rc), "bad")
            self.queue = []
            return
        self._next()
    ...
    def _next(self):
        while self.queue:
            ...
            if self.cards[key]["do"].get():
                self._start(key)
                return
        self.running = None

    def run_all(self):
        """Every ticked step, in order, stopping at the first failure."""
        self.queue = [k for k, _t, _s, _w, _a in FLOW_STEPS]
        self._next()
    ...
    def _build(self):
        ...
        tk.Label(head, text="Five steps, in order. Each one is filled in from "
                            "the settings below, so they cannot disagree with "
                            "each other. The sequence stops at the first "
                            "failure.",
                 bg=BG, fg=INK, anchor="w", justify="left",
                 wraplength=760).pack(fill="x", pady=(2, 8))

        box = tk.Frame(self, bg=BG)
        box.pack(fill="x", padx=10)
        for lbl, var in (("Dataset", self.v_data), ("Results folder", self.v_out),
                         ("Buffer", self.v_buffer),
                         ("Conditioning variable", self.v_condition),
                         ("Epochs", self.v_epochs)):
            row = tk.Frame(box, bg=BG)
            row.pack(fill="x", pady=2)
            tk.Label(row, text=lbl, width=22, anchor="w", bg=BG,
                     fg=INK).pack(side="left")
            tk.Entry(row, textvariable=var).pack(side="left", fill="x", expand=True)
        tk.Checkbutton(box, text="also use the pore size maps",
                       variable=self.v_geom, bg=BG, fg=INK,
                       anchor="w").pack(fill="x", pady=(2, 8))

        for key, title, sub, why, _act in FLOW_STEPS:
            card = tk.Frame(self, bg=BG, highlightthickness=1)
            card.pack(fill="x", padx=10, pady=3)
            top = tk.Frame(card, bg=BG)
            top.pack(fill="x")
            tk.Checkbutton(top, text=title, variable=self.cards[key]["do"],
                           bg=BG, fg=INK, anchor="w").pack(side="left")
            tk.Button(top, text="Run this step",
                      command=lambda k=key: self.run_one(k)).pack(side="right")
            tk.Label(card, text=sub + "  " + why, bg=BG, fg=INK, anchor="w",
                     justify="left", wraplength=740).pack(fill="x")

        tk.Button(self, text="Run the whole pipeline",
                  command=self.run_all).pack(anchor="w", padx=10, pady=8)


def _action_of(step_key):
    """The action a step runs."""
    for k, _t, _s, _w, act in FLOW_STEPS:
        if k == step_key:
            return act
...
class ActionPage(tk.Frame):
    ...
    def _update_computed(self):
        ...
        def num(key, default):
            ...
            try:
                ...
                return default
        def listed(mode_key, val_key):
            """How many numbers were typed in, when that box is the one in use."""
            m = next((x for x in self.action.fields if x.key == mode_key), None)
            v = next((x for x in self.action.fields if x.key == val_key), None)
            if m is None or v is None or str(m.value()).strip() != "exact numbers":
                return 0
            return len([t for t in str(v.value()).replace(",", " ").split() if t])

        ng = num("n_geom", 0)
        # Conditions per rock: what was typed in the box, or -- when it is
        # blank because exact numbers were listed -- every combination of them,
        # which is what the generator itself will work out.
        n_pe, n_da = listed("pe_mode", "pe_values"), listed("da_mode", "da_values")
        combos = max(n_pe, 1) * max(n_da, 1) if (n_pe or n_da) else 0
        ns = num("n_sets", 0) or combos
        how = ""
        if combos and not num("n_sets", 0):
            bits = []
            if n_pe:
                bits.append("%d Peclet" % n_pe)
            if n_da:
                bits.append("%d Damkohler" % n_da)
            how = "  (%s)" % " x ".join(bits)
        n_phi = listed("phi_mode", "phi_values")
        txt = ("THAT IS %d SIMULATIONS:  %d rocks  x  %d conditions each.%s\n"
               "%d flow solves, one per rock, and %d transport solves, one "
               "per simulation." % (ng * ns, ng, ns, how, ng, ng * ns))
        if n_phi and ng:
            each, extra = divmod(ng, n_phi)
            txt += ("\nThe %d porosities you listed are used in turn: %d rock%s "
                    "at each%s." % (n_phi, each, "" if each == 1 else "s",
                                    "" if not extra else
                                    ", and %d of them get one more" % extra))
    ...
    def load_settings_file(self):
        """Put a settings file into this page's "load everything from a file"
        box, from the File menu rather than from the box itself.

        Two people in a row went looking for this in the menu before they found
        the box, so it is in both places. A page that has no such box says so
        instead of doing nothing, because a menu item that silently does
        nothing looks exactly like a broken window.
        """
        f = next((x for x in self.action.fields
                  if x.key == "settings" and x.kind == "file"), None)
        if f is None:
            messagebox.showinfo(
                APP, "This page does not read a settings file.\n\n"
                     "The pages that do are the two that RUN simulations: "
                     "'Run 3D simulations to train on' and 'Run 2D simulations "
                     "to train on', under Simulate in the pipeline on the left. "
                     "Open one of those and the box is the second one down, "
                     "under 'Start from a settings file'.")
            return
        self._pick_open(f)
        if str(f.var.get()).strip():
            self.log_hint("Loaded %s. Every box below is filled from it, and "
                          "anything you type still wins over the file. Tick "
                          "'Just check the numbers, do not simulate' to see "
                          "what it means before running it."
                          % os.path.basename(str(f.var.get())))

    def log_hint(self, text):
        """A one-line note in the log, if this page is attached to one."""
        app = getattr(self, "app", None)
        if app is not None and hasattr(app, "logline"):
            try:
                app.logline(text, "hdr")
            except Exception:                                  # noqa: BLE001
                pass
...
# =============================================================================
# =============================================================================
#  A VOLUME RENDERER THAT NEEDS NOTHING BUT NUMPY AND SCIPY
# =============================================================================
# The setup page installs numpy, scipy, h5py and matplotlib. Marching cubes
# lives in scikit-image, which is not one of them, and a Poly3DCollection of
# the hundred thousand quads a real pore structure produces is not a picture
# anyone waits for. So the volume is drawn directly, by projecting the voxels.
#
# Two ways of looking, because a rock and a plume want different things:
#
#   SOLID   the nearest voxel along each line of sight wins, and is lit from a
#           real surface normal. This is what a pore structure should look
#           like: an object with shape, not a silhouette.
#
#   CLOUD   the volume is cut into slabs from near to far and composited with
#           opacity, so what is inside shows through what is in front of it.
#           This is what a concentration field should look like. Taking the
#           largest value along the ray instead, which is the cheap way, gives
#           a flat slab of one colour the moment the tracer has filled the
#           domain, and that is exactly when you most want to see inside.
#
# Three things carry the quality, and all three were missing from the first
# attempt:
#
#   Lighting from the VOLUME's own gradient, not from the depth map. A pore
#   structure is rough at the voxel scale, so the depth map's gradient is
#   noise. The gradient of the slightly smoothed occupancy is the actual
#   surface normal, and lighting from it is the difference between a green
#   slab and something with shape.
#
#   Supersampling. Voxels land on integer pixels, so every edge is a staircase
#   until the picture is drawn large and averaged down.
#
#   A box around it. Without one there is nothing to tell the eye which way is
#   x, and an unlabelled lump is not a picture of anything.
# =============================================================================


def _view_axes(elev, azim):
    """The three unit vectors of the viewer's frame, as rows.

    Row 0 goes right across the screen, row 1 goes up it, row 2 points from the
    scene towards the viewer.
    """
    import numpy as np
    a, e = np.radians(azim), np.radians(elev)
    ca, sa, ce, se = np.cos(a), np.sin(a), np.cos(e), np.sin(e)
    right = np.array([ca, sa, 0.0], np.float32)
    up = np.array([-sa * se, ca * se, ce], np.float32)
    towards = np.array([-sa * ce, ca * ce, -se], np.float32)
    return right, up, towards


def _project(shape, elev, azim):
    """Every voxel centre in the viewer's frame: across, up, and depth."""
    import numpy as np
    nx, ny, nz = shape
    right, up, towards = _view_axes(elev, azim)
    x = (np.arange(nx, dtype=np.float32) - (nx - 1) / 2.0)
    y = (np.arange(ny, dtype=np.float32) - (ny - 1) / 2.0)
    z = (np.arange(nz, dtype=np.float32) - (nz - 1) / 2.0)
    X, Y, Z = np.meshgrid(x, y, z, indexing="ij")
    u = X * right[0] + Y * right[1] + Z * right[2]
    v = X * up[0] + Y * up[1] + Z * up[2]
    d = X * towards[0] + Y * towards[1] + Z * towards[2]
    return u.ravel(), v.ravel(), d.ravel()


def _normals(keep):
    """A surface normal per voxel, from the gradient of the smoothed occupancy.

    The raw occupancy is a block of ones and zeros, and its gradient only knows
    six directions. Smoothing it first turns the staircase into a slope, and
    the slope points the way the surface actually faces.
    """
    import numpy as np
    try:
        from scipy import ndimage
        soft = ndimage.uniform_filter(keep.astype(np.float32), size=3)
    except Exception:                                          # noqa: BLE001
        soft = keep.astype(np.float32)
    gx, gy, gz = np.gradient(soft)
    n = np.sqrt(gx * gx + gy * gy + gz * gz) + 1e-9
    return (-gx / n).ravel(), (-gy / n).ravel(), (-gz / n).ravel()


def _frame_box(img, shape, elev, azim, u0, v0, scale, pad, colour=(0.55, 0.58, 0.62)):
    """A wireframe of the domain, drawn straight into the picture.

    Twelve lines. Without them the render is a lump floating on white and
    nothing says which way the flow went.
    """
    import numpy as np
    nx, ny, nz = shape
    right, up, _ = _view_axes(elev, azim)
    corners = np.array([[i * (nx - 1) - (nx - 1) / 2.0,
                         j * (ny - 1) - (ny - 1) / 2.0,
                         k * (nz - 1) - (nz - 1) / 2.0]
    ...
    cv = corners @ up
    px = img.shape[0]
    P = [(int(np.clip((cu[i] - u0) * scale + pad, 0, px - 1)),
          int(np.clip((cv[i] - v0) * scale + pad, 0, px - 1)))
         for i in range(8)]
    ...
    for a, b in edges:
        (ua, va), (ub, vb) = P[a], P[b]
        n = max(abs(ub - ua), abs(vb - va), 1)
        uu = np.linspace(ua, ub, n + 1).astype(np.int32)
        vv = np.linspace(va, vb, n + 1).astype(np.int32)
        img[vv, uu] = img[vv, uu] * 0.35 + col * 0.65
    return img


def render_volume(vol, keep, vmin, vmax, cmap, elev=22.0, azim=-58.0,
                  mode="solid", scale=2.0, super_sample=2, opacity=0.10,
                  flat_colour=None, bg=(1.0, 1.0, 1.0), box=True):
    """One picture of a volume.

    vol   the values, on a three dimensional grid
    keep  which voxels take part. For a field this is the pore space; for a
          pore structure it is the solid, since that is the object being
          looked at.
    mode  'solid' or 'cloud', described at the top of this file.
    """
    import numpy as np
    import matplotlib
    ...
    if keep.shape != vol.shape:
        keep = np.isfinite(vol)
    sel = keep.ravel() & np.isfinite(vol.ravel())
    if not sel.any():
        return None

    u, v, d = _project(vol.shape, elev, azim)
    u, v, d, val = u[sel], v[sel], d[sel], vol.ravel()[sel]

    ss = max(1, int(super_sample))
    step = scale * ss
    u0, v0 = u.min(), v.min()
    span = max(u.max() - u0, v.max() - v0, 1e-6)
    pad = int(6 * ss)
    px = int(min(span * step + 2 * pad, 1600))
    uu = np.clip(((u - u0) * step + pad).astype(np.int32), 0, px - 1)
    vv = np.clip(((v - v0) * step + pad).astype(np.int32), 0, px - 1)
    ...
    norm = np.clip((val - vmin) / rng, 0.0, 1.0)

    # One voxel covers `step` pixels, so it is painted as a block of that size.
    # Painting it as a single pixel leaves holes wherever the projection pulls
    # neighbours apart, and the holes read as gaps in the object.
    blk = max(1, int(round(step)))
    offs = [(dv, du) for dv in range(blk) for du in range(blk)]

    if mode == "cloud":
        img = _render_cloud(px, uu, vv, d, norm, offs, mapper, opacity, bg)
    else:
        img = _render_solid(px, uu, vv, d, norm, offs, mapper, sel,
                            keep, elev, azim, flat_colour, bg)

    if box:
        _frame_box(img, vol.shape, elev, azim, u0, v0, step, pad)

    if ss > 1:                       # average the supersampled picture down
        m = (px // ss) * ss
        img = img[:m, :m].reshape(m // ss, ss, m // ss, ss, 3).mean((1, 3))
    return np.clip(img, 0.0, 1.0)


def _render_solid(px, uu, vv, d, norm, offs, mapper, sel, keep, elev, azim,
                  flat_colour, bg):
    """Nearest voxel wins, lit from the surface it sits on."""
    import numpy as np
    nx_, ny_, nz_ = _normals(keep)
    nx_, ny_, nz_ = nx_[sel], ny_[sel], nz_[sel]

    o = np.argsort(-d, kind="stable")          # far first, so near overwrites
    value = np.full((px, px), np.nan, np.float32)
    NX = np.zeros((px, px), np.float32)
    NY = np.zeros((px, px), np.float32)
    NZ = np.zeros((px, px), np.float32)
    for dv, du in offs:
        vi = np.minimum(vv[o] + dv, px - 1)
        ui = np.minimum(uu[o] + du, px - 1)
        value[vi, ui] = norm[o]
        NX[vi, ui] = nx_[o]
        NY[vi, ui] = ny_[o]
        NZ[vi, ui] = nz_[o]

    # The light sits over the viewer's left shoulder, so a face turned towards
    # the viewer is bright and one turned away falls into shade.
    right, up, towards = _view_axes(elev, azim)
    light = 0.55 * towards - 0.62 * right + 0.56 * up
    light /= np.linalg.norm(light)
    lam = NX * light[0] + NY * light[1] + NZ * light[2]
    shade = np.clip(0.34 + 0.66 * np.clip(lam, 0.0, 1.0), 0.0, 1.0)

    hit = np.isfinite(value)
    if flat_colour is not None:
        rgb = np.empty((px, px, 3), np.float32)
        rgb[...] = np.asarray(flat_colour, np.float32)
    else:
        rgb = mapper(np.nan_to_num(value))[..., :3].astype(np.float32)
    rgb *= shade[..., None]
...
def _render_cloud(px, uu, vv, d, norm, offs, mapper, opacity, bg, slabs=56):
    """Front to back compositing, so the inside shows through the outside.

    The volume is cut into slabs of equal depth. Each slab is flattened into
    one image, and the images are laid over each other from near to far, each
    letting through whatever the ones in front of it did not cover. That is an
    ordinary volume rendering, and it is what makes a front INSIDE a rock
    visible from outside it.
    """
    import numpy as np
    lo, hi = float(d.min()), float(d.max())
    idx = np.clip(((d - lo) / max(hi - lo, 1e-9) * (slabs - 1)).astype(np.int32),
                  0, slabs - 1)

    acc = np.zeros((px, px, 3), np.float32)
    alpha = np.zeros((px, px), np.float32)
    # A slab near the front of the volume is drawn first, so `towards` points
    # at the viewer and the largest depth is the nearest slab.
    for s in range(slabs - 1, -1, -1):
        m = idx == s
        if not m.any():
            continue
        layer = np.full((px, px), np.nan, np.float32)
        for dv, du in offs:
            layer[np.minimum(vv[m] + dv, px - 1),
                  np.minimum(uu[m] + du, px - 1)] = norm[m]
        hit = np.isfinite(layer)
        if not hit.any():
            continue
        a = np.zeros((px, px), np.float32)
        # Opacity rises with the value, so an empty part of the pore space is
        # nearly clear and a strong one blocks what is behind it.
        # Opacity per slab has to be SMALL. At nine tenths the first slab the
        # ray meets is already solid, which is a rendering of the outer skin
        # with extra steps, and the skin is the least informative surface in
        # the volume. Low and cumulative is what lets the inside through.
        a[hit] = opacity * (0.04 + 0.96 * layer[hit] ** 2)
        c = mapper(np.nan_to_num(layer))[..., :3].astype(np.float32)
        w = (1.0 - alpha) * a
        acc += w[..., None] * c
        alpha += w
    ...
    out = np.empty((px, px, 3), np.float32)
    out[...] = np.asarray(bg, np.float32)
    out = acc + (1.0 - alpha)[..., None] * out
    return out


def render_planes(vol, pore, vmin, vmax, cmap, ix, iy, iz,
                  elev=22.0, azim=-58.0, scale=2.0, super_sample=2,
                  bg=(1.0, 1.0, 1.0), solid_colour=(0.80, 0.78, 0.75)):
    """Three cuts through the volume, standing where they belong in the box.

    This replaced an outside-only rendering, which was the wrong picture for
    this subject however well it was lit. A pore structure at forty percent
    porosity has an outer skin of pure noise, and every field is hidden behind
    it: you cannot see the geometry, you cannot see a front inside the rock,
    and you certainly cannot compare two runs. The skin is the least
    informative surface in the whole volume.

    Three orthogonal cuts show the inside directly. Each one is a full
    contrast image of real values rather than a shaded silhouette, they meet
    at a point you choose with the sliders, and the box around them says which
    way is which.

    Solid voxels on each cut are painted a flat stone colour, so the rock and
    the field are legible at once rather than one hiding the other.
    """
    import numpy as np
    import matplotlib
    ...
    if pore is None or np.asarray(pore).shape != vol.shape:
        pore = np.ones(vol.shape, bool)
    pore = np.asarray(pore, bool)

    # Only the voxels ON the three cuts are drawn. That is thirty thousand of
    # them instead of a million, so this is fast enough to drag a slider.
    take = np.zeros(vol.shape, bool)
    take[ix, :, :] = True
    take[:, iy, :] = True
    take[:, :, iz] = True
    ...
    span = max(u.max() - u0, v.max() - v0, 1e-6)
    pad = int(8 * ss)
    px = int(min(span * step + 2 * pad, 1700))
    uu = np.clip(((u - u0) * step + pad).astype(np.int32), 0, px - 1)
    vv = np.clip(((v - v0) * step + pad).astype(np.int32), 0, px - 1)
    ...
    rgb = mapper(np.nan_to_num(norm))[:, :3].astype(np.float32)
    rgb[~isp] = np.asarray(solid_colour, np.float32)
    rgb[~np.isfinite(val)] = np.asarray(solid_colour, np.float32)

    img = np.empty((px, px, 3), np.float32)
    img[...] = np.asarray(bg, np.float32)
    hit = np.zeros((px, px), bool)

    # Far first, so a nearer cut covers a further one where they overlap.
...
def _shade_from_normals(vol_for_normal, sel, elev, azim, ambient=0.42):
    """A lighting factor per selected voxel, from the field's own gradient."""
    import numpy as np
    try:
        from scipy import ndimage
        soft = ndimage.uniform_filter(vol_for_normal.astype(np.float32), size=3)
    except Exception:                                          # noqa: BLE001
        soft = vol_for_normal.astype(np.float32)
    gx, gy, gz = np.gradient(soft)
    n = np.sqrt(gx * gx + gy * gy + gz * gz) + 1e-9
    gx, gy, gz = (-gx / n).ravel()[sel], (-gy / n).ravel()[sel], (-gz / n).ravel()[sel]
    right, up, towards = _view_axes(elev, azim)
    light = 0.5 * towards - 0.6 * right + 0.62 * up
    light /= np.linalg.norm(light)
    lam = gx * light[0] + gy * light[1] + gz * light[2]
    return np.clip(ambient + (1.0 - ambient) * np.clip(lam, 0.0, 1.0), 0.0, 1.0)


def render_iso(vol, pore, level, vmin, vmax, cmap, elev=22.0, azim=-58.0,
               scale=2.0, super_sample=2, band=None, bg=(1.0, 1.0, 1.0),
               colour=None):
    """The surface where the field passes through one value.

    This is the picture that answers "where has it got to". A cloud of the
    whole field is a fog you cannot measure anything against; one surface at a
    value you chose has a definite shape, and moving the level sweeps it
    through the volume so the whole field can be read a slice of value at a
    time.

    No marching cubes involved, and none needed: the voxels whose value sits
    within a narrow band of the level ARE the surface, one voxel thick, and
    lighting them from the field's own gradient gives a smooth-looking shell
    ...
    vol = np.asarray(vol, np.float32)
    if pore is None or np.asarray(pore).shape != vol.shape:
        pore = np.isfinite(vol)
    pore = np.asarray(pore, bool)

    if band is None:
        band = 0.035 * max(vmax - vmin, 1e-9)
    sel = (pore & np.isfinite(vol) & (np.abs(vol - level) <= band)).ravel()
    if not sel.any():
        return None, 0

    u, v, d = _project(vol.shape, elev, azim)
    u, v, d = u[sel], v[sel], d[sel]
    shade = _shade_from_normals(np.where(pore, vol, vmin), sel, elev, azim)
    ...
    if colour is None:
        try:
            ...
            import matplotlib.cm as _cm
            mapper = _cm.get_cmap(cmap)
        base = np.asarray(mapper((level - vmin) / max(vmax - vmin, 1e-9))[:3],
                          np.float32)
    else:
    ...
    for dv in range(blk):
        for du in range(blk):
            img[np.minimum(vv[o] + dv, px - 1),
                np.minimum(uu[o] + du, px - 1)] = base * shade[o][:, None]
...
class Viewer(tk.Frame):
    ...
    The left side is the file itself, laid out the way an HDF5 viewer lays it
    out: every group, every array, every label. The right side shows whichever
    one is selected, and what "shows" means follows what the thing is rather
    than what the file was supposed to be:

        an array with a grid in it  ->  a picture, with sliders for the axes
                                        that are not on screen
        a short list or table       ->  the numbers
        text                        ->  the text, which is how the input file
                                        and the kinetics file are read
        a group                     ->  what is inside it and what it is called

    Nothing here requires a particular key to exist. A file with no conditions,
    no input files and no reaction fields opens and browses exactly like a full
    one; the tree is simply shorter.
    """

    # Axis names for the arrays this project writes. The last three axes of a
    # field array are always the grid, so those are named without a lookup;
    # this only covers the leading axes, and anything not listed falls back to
    # its number.
    AXIS_HINTS = {
        "samples/conc":      ["run", "snapshot", "species"],
        "samples/dconc_dt":  ["run", "snapshot", "species"],
        "samples/rate":      ["run", "snapshot", "reaction"],
        "samples/velocity":  ["run", "snapshot", "component"],
        "samples/material":  ["run", "snapshot"],
        "samples/biofilm":   ["run", "snapshot"],
        "samples/mineral":   ["run", "snapshot"],
        "samples/gdf":       ["run", "snapshot"],
        "geom/material":     ["rock"],
        "geom/gdf":          ["rock"],
        "geom/edt":          ["rock"],
        "geom/biofilm":      ["rock"],
    ...
    # A plain-language line for the parts of the layout that have one. Anything
    # missing from here simply gets no line, which is the correct behaviour for
    # a file that carries something this GUI has never heard of.
    WHAT = {
        "geom": "the rocks, as they started. One entry per rock.",
        "samples": "the runs. One entry each.",
        "inputs": "the files each run was given, kept word for word.",
        "provenance": "where each number came from.",
        "ancillary": "context: notes, porosity, permeability.",
        "geom/material": "the pore structure. 0 solid, 1 interface, 2 pore.",
        "geom/gdf": "distance from the inlet, measured through pore space.",
        "geom/edt": "distance to the nearest solid.",
        "geom/biofilm": "which voxels counted as biofilm at the start.",
        "geom/gid": "identity number of each rock.",
        "samples/conc": "every species field: the chemicals and the microbes.",
        "samples/conc_scale": "one divisor per species. Multiply to get real values.",
        "samples/dconc_dt": "how fast each species was changing.",
        "samples/rate": ("how fast each reaction ran, one channel per reaction. Stored divided by rate_scale, the same way conc is."),
        "samples/rate_scale": "one divisor per reaction channel.",
        "samples/velocity": "the flow field.",
        "samples/params": "the conditions of each run, one row per run.",
        "samples/t_norm": "time of each snapshot, 0 to 1.",
        "samples/t_seconds": "the same times in seconds.",
        "samples/geom_index": "which rock each run used.",
        "inputs": "what each run was TOLD to do. Four tables.",
        "inputs/xml": "every setting in the run's input file, as a table. The "
                      "number column is a real number, so it is usable as it "
                      "stands.",
        "inputs/kinetics": "every rate constant in the .hh files, with the note "
                           "written beside it in the source.",
        "inputs/order": "the chemical order those files expect. Compare it "
                        "against species.",
        "inputs/files": "which file each run got, or why it got none.",
        "inputs/time_step": "the step the run used, and the step its kinetics "
                            "file assumes. They have to agree.",
        "grid": "what was imposed at the edges of the box.",
        "grid/boundary_conditions": "per species, at the inlet and the outlet.",
        "ancillary": "free text. Nothing computed lives here.",
        "ancillary/run_note": "whatever the collector had to say about a run.",
        "ancillary/run_name": "the folder each run came from.",
        "samples/run_id": "identity number of each run.",
    }
    ...
    def __init__(self, parent, app):
        ...
        self.fig = None
        self._figs = []         # every figure made here, so none is orphaned
        self._pending = None    # a redraw waiting to happen, see redraw()
        self.nodes = {}         # tree iid -> node dict
        self.sel = None         # the selected node
        self._h = None          # open h5py.File, closed when another opens

        self.var_cmap = tk.StringVar(value="viridis")
        self.var_inspect = tk.BooleanVar(value=False)
        self.var_hide_solid = tk.BooleanVar(value=True)
        self.var_log = tk.BooleanVar(value=False)
        self.var_view = tk.StringVar(value="Plane")
        self.var_level = tk.DoubleVar(value=0.5)
        self.var_opacity = tk.DoubleVar(value=0.10)
        self.var_trim = tk.BooleanVar(value=True)
        self.var_elev = tk.IntVar(value=22)
        self.var_azim = tk.IntVar(value=-58)
        self.ax_x = tk.IntVar(value=0)
        self.ax_y = tk.IntVar(value=0)
        self.ax_pos = []        # one IntVar per axis
        self._axes_for = None   # the array the axis choice above belongs to

        bar = tk.Frame(self, bg=BG)
        bar.pack(fill="x", padx=8, pady=6)
        tk.Label(bar, text="Open:", bg=BG, fg=INK,
                 font=base_font(9)).pack(side="left")
        for label, fn in (("Dataset (.h5)", self.open_dataset),
                          ("Geometry (.npz)", self.open_geometry),
        ...
        tk.Checkbutton(bar, text="Inspect", variable=self.var_inspect,
                       bg=BG, fg=INK, font=base_font(8),
                       activebackground=BG, selectcolor=PANEL,
                       command=self.redraw).pack(side="right", padx=8)

        pan = tk.PanedWindow(self, orient="horizontal", bg=BG, sashwidth=6,
                             sashrelief="flat", bd=0)
        pan.pack(fill="both", expand=True, padx=8, pady=(0, 8))

        left = tk.Frame(pan, bg=PANEL, highlightbackground=LINE,
                        highlightthickness=1)
        self.head = tk.Label(left, bg=HEAD, fg=INK, anchor="w", padx=8, pady=5,
                             font=mono_font(9), text="  no file open")
        self.head.pack(fill="x")
        tsc = tk.Frame(left, bg=PANEL)
        tsc.pack(fill="both", expand=True)
        self.tree = ttk.Treeview(tsc, show="tree", selectmode="browse")
        sb = ttk.Scrollbar(tsc, orient="vertical", command=self.tree.yview)
        self.tree.configure(yscrollcommand=sb.set)
        sb.pack(side="right", fill="y")
        self.tree.pack(side="left", fill="both", expand=True)
        self.tree.bind("<<TreeviewSelect>>", self._on_select)
        pan.add(left, minsize=240, width=340)

        right = tk.Frame(pan, bg=BG)
        self.ctl = tk.Frame(right, bg=PANEL, highlightbackground=LINE,
                            highlightthickness=1)
        self.ctl.pack(fill="x")
        self.plotarea = tk.Frame(right, bg=PANEL, highlightbackground=LINE,
                                 highlightthickness=1)
        self.plotarea.pack(fill="both", expand=True, pady=(8, 0))
        pan.add(right, minsize=420)

        self._blank("  Nothing open yet.\n\n"
                    "  Open a dataset to walk through everything in it: the pore\n"
                    "  structures, the fields through time, the flow, the conditions,\n"
                    "  and the input file each run was given.\n\n"
                    "  Nothing has to be present. Whatever the file holds is what\n"
                    "  the tree shows.\n")

    # ------------------------------------------------------------------ util
    def _drop_fig(self):
        """Let go of the picture that was on screen.

        matplotlib keeps every figure made through pyplot until it is closed,
        so a session of clicking around the tree quietly accumulates hundreds
        of them. It shows up first as a warning and later as a viewer that
        gets slower the longer it is open.
        """
        # Closing self.fig alone is not enough. A draw that fails part way
        # through has already created its figure and never got as far as
        # assigning it, so that one is orphaned and no reference to it is left
        # anywhere except inside matplotlib. Keeping the list is what makes
    ...
    def _blank(self, text):
        ...
        for w in self.plotarea.winfo_children():
            w.destroy()
        tk.Label(self.plotarea, bg=PANEL, fg=MUTE, justify="left",
                 font=base_font(10), text=text).pack(fill="both", expand=True)
    ...
    @staticmethod
    def _text_of(v):
        """Anything HDF5 hands back for a string, as a str.

        There are four shapes this arrives in and they are easy to confuse: a
        plain bytes, a numpy bytes_, a str, and a zero-dimensional array
        wrapping any of those. The last one is what a single stored file comes
        back as, and str() on it produces b\'...\\n...\' on one line: the whole
        input file as an unreadable escape sequence. Unwrap first, decode
        second.
        """
        import numpy as np
    ...
    def _channel_names(self, axis_name):
        """The names along a labelled axis, taken from the file's own labels.

        The species axis of samples/conc is named by the file attribute
        'species', and the reaction axis of samples/rate by 'reactions'. Both
        were written by whoever collected the file, which in turn got them from
        the input file and from the kinetics header. Showing the name beside
        the index is the difference between "reaction 0" and
        "R_Bio_growth_on_Ac_and_A".
        """
        key = {"species": "species", "reaction": "reactions"}.get(axis_name)
        d = self.data
        if not key or not d or d["kind"] != "h5":
            return []
        try:
            v = d["h"].attrs.get(key)
            if v is None:
    ...
    def _held_labels(self, path, shape, skip=()):
        """"snapshot 3, species 2 (P)" for every axis being held at one value."""
        names = self._axis_names(path, shape)
        out = []
        for i in range(len(shape)):
            if i in skip or int(shape[i]) <= 1:
                continue
            v = int(self.ax_pos[i].get())
            lab = "%s %d" % (names[i], v)
            nm = self._channel_names(names[i])
            if nm and v < len(nm):
                lab += " (%s)" % nm[v]
            out.append(lab)
        return out

    def _axis_names(self, path, shape):
        """A name per axis. The last three are the grid; the rest are looked up
        and fall back to their number, so an unknown array still browses."""
        n = len(shape)
        names = ["axis %d" % i for i in range(n)]
        if n >= 3:
            names[-3], names[-2], names[-1] = "x", "y", "z"
        lead = self.AXIS_HINTS.get(path.strip("/"), [])
    ...
    def _fill_tree(self):
        ...
        if not d:
            return
        root = self.tree.insert("", "end", text=d["title"], open=True)
        self.nodes[root] = dict(kind="root", path="", label=d["title"])

        if d["kind"] == "h5":
            self._fill_h5(root, d["h"], "")
        else:
            for name, arr in d["arrays"].items():
                iid = self.tree.insert(root, "end",
                                       text="%s   %s" % (name, self._dimlab(arr.shape)))
                self.nodes[iid] = dict(kind="array", path=name, obj=arr,
                                       shape=tuple(arr.shape), dtype=arr.dtype,
                                       attrs={})
        # Land on something worth looking at rather than on the root.
        first = self._first_drawable()
        if first:
    ...
    @staticmethod
    def _dimlab(shape):
        return "(" + ", ".join(str(int(s)) for s in shape) + ")" if shape else "scalar"

    def _fill_h5(self, parent, grp, prefix):
        import h5py
        # Labels on the group itself come first: they are facts about
        # everything below, so they belong above it.
        for k in sorted(grp.attrs.keys()):
            iid = self.tree.insert(parent, "end", text="· %s" % k)
            self.nodes[iid] = dict(kind="attr", path=prefix + "@" + k,
                                   value=grp.attrs[k], label=k)
        for k in sorted(grp.keys()):
            try:
                o = grp[k]
            except Exception as e:                             # noqa: BLE001
                iid = self.tree.insert(parent, "end", text="%s   (unreadable)" % k)
                self.nodes[iid] = dict(kind="broken", path=prefix + k, why=str(e))
                continue
            path = prefix + k
            if isinstance(o, h5py.Group):
                iid = self.tree.insert(parent, "end", text=k + "/",
                                       open=(prefix == "" and k in ("geom", "samples")))
                self.nodes[iid] = dict(kind="group", path=path, obj=o,
                                       attrs=dict(o.attrs))
                self._fill_h5(iid, o, path + "/")
            else:
                iid = self.tree.insert(
                    parent, "end", text="%s   %s" % (k, self._dimlab(o.shape)))
                self.nodes[iid] = dict(kind="array", path=path, obj=o,
                                       shape=tuple(o.shape), dtype=o.dtype,
                                       attrs=dict(o.attrs))

    def _first_drawable(self):
        """Prefer a field with a grid in it, so the viewer opens on a picture."""
        best = None
        for iid, n in self.nodes.items():
            if n["kind"] != "array":
                continue
            if len(n.get("shape", ())) >= 3 and self._is_numeric(n):
                p = n["path"]
                if p.endswith("samples/conc"):
                    return iid
                if best is None:
                    best = iid
        return best or next((i for i, n in self.nodes.items()
                             if n["kind"] == "array"), None)

    @staticmethod
    def _is_numeric(n):
        return n.get("dtype") is not None and n["dtype"].kind in "fiub"

    @staticmethod
    def _is_compound(n):
        """A real table: an HDF5 compound dataset, with named columns.

        This is how the input files are stored. One node holds the whole
        table, so opening it is one click rather than a walk through a group
        of parallel arrays that have to be lined up by hand.
        """
        d = n.get("dtype")
        return d is not None and d.kind == "V" and d.names

    @staticmethod
    def _is_text(n):
        d = n.get("dtype")
        return d is not None and d.kind in "SOU"
    ...
    def _build_controls(self):
        ...
        r1.pack(fill="x", pady=(5, 2))
        tk.Label(r1, text=n.get("path") or n.get("label", ""), bg=PANEL, fg=INK,
                 font=mono_font(10)).pack(side="left", padx=8)
        what = self.WHAT.get((n.get("path") or "").strip("/"))
        if what:
            tk.Label(r1, text=what, bg=PANEL, fg=MUTE,
                     font=base_font(8)).pack(side="left", padx=8)

        if self._is_compound(n) and len(n["shape"]) == 2:
            if not self.ax_pos or len(self.ax_pos) != 1:
                self.ax_pos = [tk.IntVar(value=0)]
            r3 = tk.Frame(self.ctl, bg=PANEL)
            r3.pack(fill="x", pady=(0, 6))
            hi = int(n["shape"][0]) - 1
            tk.Label(r3, text="run", bg=PANEL, fg=INK,
                     font=base_font(8)).pack(side="left", padx=(8, 2))
            sp = tk.Spinbox(r3, from_=0, to=hi, width=5, font=mono_font(8),
                            textvariable=self.ax_pos[0], command=self.redraw)
            sp.pack(side="left")
            sp.bind("<Return>", lambda e: self.redraw())
            tk.Label(r3, text="of %d" % hi, bg=PANEL, fg=MUTE,
                     font=base_font(8)).pack(side="left", padx=(2, 6))
            return

        if n["kind"] != "array" or not self._is_numeric(n):
            return
        shape = n["shape"]
        if len(shape) < 2 or self._as_table(n):
            return
        if self.var_inspect.get():
            return

        names = self._axis_names(n["path"], shape)
        solid_3d = len(shape) >= 3 and all(int(v) > 1 for v in shape[-3:])
        if not solid_3d:
            self.var_view.set("Plane")

        # Which two axes are on screen. Defaults are the two grid axes that
        # give the picture people actually want: x across, y up, and whatever
        # is left on a slider. That works for a 2D field, whose last axis is
        # simply of length one, without asking anyone to pick a mode.
        #
        # These are reset whenever the selection MOVES to a different array.
        # Carrying one array's choice over to the next looks like it preserves
        # the user's setting, but the axes of a six-axis field mean nothing on
        # a four-axis one: it silently drew snapshot against species and called
        # it a picture.
        if self._axes_for != n["path"] or len(self.ax_pos) != len(shape):
            if len(shape) >= 3:
                self.ax_x.set(len(shape) - 3); self.ax_y.set(len(shape) - 2)
            else:
                self.ax_x.set(1); self.ax_y.set(0)
            # Where each held axis starts. The middle of z, because a slice
            # through the centre of a rock is the one worth seeing first; and
            # the LAST snapshot, because snapshot 0 is the state before
            # anything has entered the sample and is a blank picture.
            starts = []
            for i in range(len(shape)):
                if i == len(shape) - 1:
                    starts.append(int(shape[i]) // 2)
                elif names[i] == "snapshot":
                    starts.append(int(shape[i]) - 1)
                else:
                    starts.append(0)
            self.ax_pos = [tk.IntVar(value=v) for v in starts]
            self._axes_for = n["path"]

        r2 = tk.Frame(self.ctl, bg=PANEL)
        r2.pack(fill="x", pady=(0, 3))
        in3d = solid_3d and self.var_view.get() != "Plane"
        tk.Label(r2, text=("Whole volume:" if in3d else "On screen:"),
                 bg=PANEL, fg=INK, font=base_font(8)).pack(side="left", padx=(8, 4))
        for lab, var in ([] if in3d else (("across", self.ax_x), ("up", self.ax_y))):
            tk.Label(r2, text=lab, bg=PANEL, fg=MUTE,
                     font=base_font(8)).pack(side="left", padx=(6, 2))
            cb = ttk.Combobox(r2, state="readonly", width=13, font=base_font(8),
                              values=["%s  (%d)" % (names[i], shape[i])
                                      for i in range(len(shape))])
            cb.current(min(var.get(), len(shape) - 1))
            cb.pack(side="left")
            cb.bind("<<ComboboxSelected>>",
                    lambda e, v=var, c=cb: (v.set(c.current()),
                                            self._build_controls(), self.redraw()))

        if solid_3d:
            tk.Label(r2, text="View:", bg=PANEL, fg=INK,
                     font=base_font(8)).pack(side="left", padx=(12, 2))
            cbv = ttk.Combobox(r2, textvariable=self.var_view, width=9,
                               state="readonly", font=base_font(8),
                               values=["Plane", "Cloud", "Surface", "Solid"])
            cbv.pack(side="left")
            cbv.bind("<<ComboboxSelected>>",
                     lambda e: (self._build_controls(), self.redraw()))

        tk.Label(r2, text="Colours:", bg=PANEL, fg=INK,
                 font=base_font(8)).pack(side="left", padx=(12, 2))
        cb3 = ttk.Combobox(r2, textvariable=self.var_cmap, width=9,
                           state="readonly", font=base_font(8),
                           values=["viridis", "magma", "plasma", "inferno",
                                   "cividis", "coolwarm", "YlGnBu", "Greys"])
        cb3.pack(side="left")
        cb3.bind("<<ComboboxSelected>>", lambda e: self.redraw())

        tk.Checkbutton(r2, text="log", variable=self.var_log, bg=PANEL, fg=INK,
                       font=base_font(8), activebackground=PANEL,
                       selectcolor=PANEL, command=self.redraw).pack(side="left",
                                                                   padx=(10, 0))
        if self._mask_available(n):
            tk.Checkbutton(r2, text="hide the solid", variable=self.var_hide_solid,
                           bg=PANEL, fg=INK, font=base_font(8),
                           activebackground=PANEL, selectcolor=PANEL,
                           command=self.redraw).pack(side="left", padx=(8, 0))

        if solid_3d and self.var_view.get() != "Plane":
            view = self.var_view.get()
            if view == "Surface":
                # The one control that matters in this view. Dragging it
                # sweeps the surface through the volume, which is how a whole
                # field gets read one value at a time.
                tk.Label(r2, text="level", bg=PANEL, fg=INK,
                         font=base_font(8)).pack(side="left", padx=(10, 2))
                sc = tk.Scale(r2, from_=0.0, to=1.0, resolution=0.01,
                              orient="horizontal", length=130, showvalue=True,
                              variable=self.var_level, bg=PANEL, fg=INK,
                              troughcolor=HEAD, highlightthickness=0,
                              font=base_font(7), command=lambda e: self.redraw())
                sc.pack(side="left")
            elif view == "Cloud":
                tk.Label(r2, text="haze", bg=PANEL, fg=INK,
                         font=base_font(8)).pack(side="left", padx=(10, 2))
                sc = tk.Scale(r2, from_=0.02, to=0.60, resolution=0.02,
                              orient="horizontal", length=110, showvalue=True,
                              variable=self.var_opacity, bg=PANEL, fg=INK,
                              troughcolor=HEAD, highlightthickness=0,
                              font=base_font(7), command=lambda e: self.redraw())
                sc.pack(side="left")
            tk.Checkbutton(r2, text="trim outliers", variable=self.var_trim,
                           bg=PANEL, fg=INK, font=base_font(8),
                           activebackground=PANEL, selectcolor=PANEL,
                           command=self.redraw).pack(side="left", padx=(10, 0))
            for lab, var in (("turn", self.var_azim), ("tilt", self.var_elev)):
                tk.Label(r2, text=lab, bg=PANEL, fg=INK,
                         font=base_font(8)).pack(side="left", padx=(8, 2))
                sp = tk.Spinbox(r2, from_=-180, to=180, increment=15, width=5,
                                font=mono_font(8), textvariable=var,
                                command=self.redraw)
                sp.pack(side="left")
                sp.bind("<Return>", lambda e: self.redraw())

        if in3d:
            # The three grid axes are all on screen at once, so only the run,
            # the snapshot and the species are left to choose.
            free = list(range(len(shape) - 3))
        else:
            free = [i for i in range(len(shape))
                    if i not in (self.ax_x.get(), self.ax_y.get())]
        if free:
            r3 = tk.Frame(self.ctl, bg=PANEL)
            r3.pack(fill="x", pady=(0, 6))
            for i in free:
                hi = int(shape[i]) - 1
                tk.Label(r3, text="%s" % names[i], bg=PANEL, fg=INK,
                         font=base_font(8)).pack(side="left", padx=(8, 2))
                if self.ax_pos[i].get() > hi:
                    self.ax_pos[i].set(hi)
                if hi == 0:
                    tk.Label(r3, text="0", bg=PANEL, fg=MUTE,
                             font=mono_font(8)).pack(side="left")
                    continue
                s = tk.Spinbox(r3, from_=0, to=hi, width=5, font=mono_font(8),
                               textvariable=self.ax_pos[i], command=self.redraw)
                s.pack(side="left")
                s.bind("<Return>", lambda e: self.redraw())
                tk.Label(r3, text="of %d" % hi, bg=PANEL, fg=MUTE,
                         font=base_font(8)).pack(side="left", padx=(2, 2))
                # the name of whatever is at this index, when the file says
                chn = self._channel_names(names[i])
                cur = int(self.ax_pos[i].get())
                if chn and cur < len(chn):
                    tk.Label(r3, text=chn[cur], bg=PANEL, fg=INK,
                             font=base_font(8)).pack(side="left", padx=(0, 6))
            tk.Button(r3, text="Redraw", font=base_font(8), padx=8,
                      command=self.redraw).pack(side="right", padx=8)

    def _as_table(self, n):
        """Small numbers are numbers, not pictures. A run's conditions and the
        snapshot times are read, not looked at.

        The test is on the WIDTH, not the total size. A table of conditions is
        a table whether it has six runs in it or six hundred, and drawing one
        as a heatmap of six columns tells nobody anything. Field arrays never
        reach here: they carry the grid on their last three axes, even in 2D
        where the third is of length one.
        """
        # A table whose columns are named is a table whatever its width. The
        # controls have to agree with the drawer about this, or a named table
        # gets axis pickers above a picture it is never going to draw.
        if self._is_compound(n) or self._paired_names(n) is not None:
            return True
        sh = [int(v) for v in n.get("shape", ())]
        if len(sh) == 0:
            return True
        if len(sh) == 1:
            return sh[0] <= 4096
    ...
    def _mask_available(self, n):
        d = self.data
        if not d or d["kind"] != "h5":
            return False
        try:
            mat = d["h"]["geom/material"]
        except Exception:                                      # noqa: BLE001
            return False
        return (len(n["shape"]) >= 3
                and tuple(n["shape"][-3:]) == tuple(mat.shape[-3:])
                and not n["path"].endswith("material"))

    # ----------------------------------------------------------- the picture
    def _slice(self, n):
        """The 2D array on screen, plus its two axis labels.

        Only the plane asked for is read. A concentration array of a hundred
        million voxels never lands in memory whole.
        """
        import numpy as np
        shape = n["shape"]
        xa, ya = int(self.ax_x.get()), int(self.ax_y.get())
        if xa == ya:                       # cannot map both to the same axis
            ya = (xa + 1) % len(shape)
            self.ax_y.set(ya)
        idx = []
        for i in range(len(shape)):
            if i in (xa, ya):
                ...
                idx.append(int(min(self.ax_pos[i].get(), int(shape[i]) - 1)))
        arr = np.asarray(n["obj"][tuple(idx)]).astype("float32")
        # The two kept axes come back in their original order. Put the one the
        # user asked for across the screen on the columns.
        if xa < ya:
            arr = arr.T
        names = self._axis_names(n["path"], shape)
        return arr, names[xa], names[ya]

    def _volume(self, n):
        """The full three-dimensional field at the held indices.

        The cut-away needs the block, not a plane. Only the leading axes are
        pinned, so one rock at one snapshot is read and nothing more.
        """
        import numpy as np
        shape = n["shape"]
        idx = [int(min(self.ax_pos[i].get(), int(shape[i]) - 1))
               for i in range(len(shape) - 3)] + [slice(None)] * 3
        return np.asarray(n["obj"][tuple(idx)]).astype("float32")

    def _volume_mask(self, n):
        """The pore mask lined up with _volume, or None if it does not apply."""
        import numpy as np
        if not (self.var_hide_solid.get() and self._mask_available(n)):
            return None
        d = self.data
        try:
            p = n["path"].strip("/")
            lead = len(n["shape"]) - 3
            g = 0
            if p.startswith("samples/") and "samples/geom_index" in d["h"] and lead:
                g = int(d["h"]["samples/geom_index"][int(self.ax_pos[0].get())])
            elif p.startswith("geom/") and lead:
                g = int(self.ax_pos[0].get())
            mat = d["h"]["geom/material"]
            m = np.asarray(mat[g] if mat.ndim == 4 else mat[...])
        except Exception:                                      # noqa: BLE001
            return None
        return (m == 2) if m.ndim == 3 else None

    def _mask_for(self, n):
        import numpy as np
        if not (self.var_hide_solid.get() and self._mask_available(n)):
            return None
        d = self.data
        ...
        try:
            ...
            p = n["path"].strip("/")
            lead = len(shape) - 3
            if p.startswith("samples/") and "samples/geom_index" in d["h"]:
                s = int(self.ax_pos[0].get()) if lead >= 1 else 0
                g = int(d["h"]["samples/geom_index"][s])
            elif p.startswith("geom/") and lead >= 1:
                g = int(self.ax_pos[0].get())
            mat = d["h"]["geom/material"]
            m = np.asarray(mat[g] if mat.ndim == 4 else mat[...])
        except Exception:                                      # noqa: BLE001
            return None
        # Slice the mask the same way the field was sliced, so it lines up
        ...
        for i in range(3):
            a = i + off
            if a in (xa, ya):
                mi.append(slice(None))
            else:
                mi.append(int(min(self.ax_pos[a].get(), m.shape[i] - 1)))
        try:
            out = m[tuple(mi)]
        except Exception:                                      # noqa: BLE001
            return None
    ...
    def redraw(self):
        """Ask for a redraw. The drawing itself happens once, shortly.

        Every control here carries command=self.redraw, and Tk fires those
        commands while the controls are being built as well as when they are
        used. Drawing immediately therefore meant THREE full renders for one
        click on the tree, which on a hundred-cubed volume is three seconds of
        work to produce one picture, and left the two thrown-away figures open
        behind it.
        """
        if not self.data or not self.sel:
            return
        if self._pending is not None:
            try:
                ...
                pass
        self._pending = self.after(70, self._redraw_now)
    ...
    def _redraw_now(self):
        ...
        try:
            ...
            self._blank("  Could not show that:\n\n    %s\n\n"
                        "  The rest of the file is still browsable." % e)

    def _draw(self):
        n = self.sel
        if n is None:
            return
        if self.var_inspect.get() or n["kind"] in ("root", "group", "attr",
                                                   "broken"):
            return self._draw_text(n)
        if n["kind"] != "array":
            return self._draw_text(n)
        if self._is_compound(n):
            return self._draw_columns(n)
        if self._paired_names(n) is not None:
            return self._draw_paired(n)
        if self._is_text(n):
            return self._draw_text(n)
        if not self._is_numeric(n):
            return self._draw_text(n)
        if self._as_table(n):
            return self._draw_table(n)
        return self._draw_image(n)
    ...
    def _text_widget(self):
        ...
        for w in self.plotarea.winfo_children():
            w.destroy()
        fr = tk.Frame(self.plotarea, bg=PANEL)
        fr.pack(fill="both", expand=True)
        t = tk.Text(fr, wrap="none", bg=PANEL, fg=INK, font=mono_font(9),
    ...
    def _describe(self, n):
        """What a thing is, in the form a viewer's inspect pane shows it."""
        import numpy as np
        out = []
        out.append("path        %s" % (n.get("path") or "/"))
        out.append("kind        %s" % n["kind"])
        if n["kind"] == "array":
            out.append("shape       %s" % (self._dimlab(n["shape"])))
            out.append("dtype       %s" % n["dtype"])
            o = n["obj"]
            ...
            try:
                out.append("on disk     %s"
                           % _human_bytes(getattr(o, "nbytes", 0)))
            except Exception:                                  # noqa: BLE001
                pass
            if self._is_numeric(n) and int(np.prod(n["shape"] or (1,))) > 0:
                try:
                    a = self._sample_values(n)
                    fin = a[np.isfinite(a)]
                    if fin.size:
                        out.append("values      min %.6g   max %.6g   mean %.6g"
                                   % (fin.min(), fin.max(), fin.mean()))
                        if fin.size < a.size:
                            out.append("            %d of %d sampled points are "
                                       "not a number" % (a.size - fin.size, a.size))
                except Exception:                              # noqa: BLE001
                    pass
        attrs = n.get("attrs") or {}
        if attrs:
            out.append("")
            out.append("labels on it")
            for k in sorted(attrs):
                out.append("  %-22s %s" % (k, _short_value(attrs[k])))
        if n["kind"] == "group":
            kids = list(n["obj"].keys())
            out.append("")
            out.append("holds %d thing%s" % (len(kids), "" if len(kids) == 1 else "s"))
            for k in kids:
                try:
                    o = n["obj"][k]
                    out.append("  %-24s %s"
                               % (k, self._dimlab(o.shape)
                                  if hasattr(o, "shape") else "group"))
                except Exception:                              # noqa: BLE001
                    out.append("  %-24s (unreadable)" % k)
        return "\n".join(out)

    def _sample_values(self, n):
        """Enough of an array to quote a range from, without reading all of it."""
        import numpy as np
        o = n["obj"]
        shape = n["shape"]
        if int(np.prod(shape)) <= 2_000_000:
            return np.asarray(o[...], dtype="float64")
        idx = tuple(0 if i < len(shape) - 3 else slice(None)
                    for i in range(len(shape)))
        return np.asarray(o[idx], dtype="float64")

    def _draw_text(self, n):
        t = self._text_widget()
        if self.var_inspect.get() or n["kind"] in ("root", "group", "broken"):
            body = self._describe(n) if n["kind"] != "broken" \
                else "path        %s\n\nThis could not be read:\n\n    %s" \
                     % (n["path"], n.get("why", ""))
            if n["kind"] == "root":
                body = self.data.get("summary", "") + "\n\n" + body
        elif n["kind"] == "attr":
            body = "%s\n\n%s" % (n["label"], _long_value(n["value"]))
        else:
            body = self._array_text(n)
        what = self.WHAT.get((n.get("path") or "").strip("/"))
        if what and not self.var_inspect.get():
            body = what + "\n\n" + body
        t.insert("1.0", body)
        t.configure(state="disabled")

    def _array_text(self, n):
        """A text array. This is how the input file and the kinetics file read:
        one long string, or one string per run."""
        import numpy as np
        a = np.asarray(n["obj"][...])
        flat = np.atleast_1d(a).reshape(-1)
        if flat.size == 1:
            return self._text_of(flat[0] if a.ndim else a)
        out = []
        for i, v in enumerate(flat[:200]):
            s = self._text_of(v)
            if "\n" in s:
                out.append("[%d]\n%s\n" % (i, s))
            else:
                out.append("[%3d]  %s" % (i, s))
        if flat.size > 200:
            out.append("... and %d more" % (flat.size - 200))
        return "\n".join(out)

    def _paired_names(self, n):
        """The list of names belonging to a two dimensional table, if the file
        carries one.

        A table of settings is a wall of numbers until each column is named,
        and the file does name them: setting_value sits beside setting_name,
        constant_value beside constant_name. Pairing them up is the difference
        between something readable and something nobody opens twice.
        """
        if n.get("kind") != "array" or len(n.get("shape", ())) != 2:
            return None
        d = self.data
        if d.get("kind") != "h5":
            return None
        ...
        for cand in (stem + "_name", stem.replace("_value", "") + "_name",
                     stem + "_names"):
            full = (parent + "/" + cand) if parent else cand
            try:
                o = d["h"][full]
            except Exception:                                  # noqa: BLE001
                continue
            if len(o.shape) == 1 and int(o.shape[0]) == int(n["shape"][1]):
                return [self._text_of(v) for v in o[...]]
        return None

    def _draw_columns(self, n):
        """A compound dataset, drawn as the table it is.

        The first axis is the run wherever there is more than one, so a run
        picker sits above the table and the rest of the columns are the table
        itself. Rows whose every column is blank are dropped: a table sized to
        the widest run leaves blanks in the narrower ones, and printing them
        just makes the reader scroll past nothing.
        """
        import numpy as np
        a = np.asarray(n["obj"][...])
        ...
        if per_run:
            if not self.ax_pos or len(self.ax_pos) != 1:
                self.ax_pos = [tk.IntVar(value=0)]
            k = int(min(self.ax_pos[0].get(), a.shape[0] - 1))
            rows = a[k]
        else:
            k, rows = None, a
        ...
        def cell(v, name):
            ...
            if isinstance(v, (float, np.floating)):
                return "" if not np.isfinite(v) else _num(v)
            return str(v)

        table = []
        for r in rows:
            vals = [cell(r[c], c) for c in cols]
            if any(v.strip() for v in vals):
                table.append(vals)

        # Width each column actually needs, capped so one long sentence cannot
        # push everything else off the screen.
        # One long value in one column must not push every other column off
        # the screen, so each is given a ceiling suited to what it holds.
        CAP = {"name": 34, "path": 44, "meaning": 54, "unit": 11,
               "value": 14, "number": 11, "where": 58, "file": 24,
               "species": 12, "face": 8, "type": 11}
        width = []
        for j, c in enumerate(cols):
            w = max([len(c)] + [len(t[j]) for t in table]) if table else len(c)
            width.append(min(w, CAP.get(c, 34)))

        out = []
        what = self.WHAT.get((n.get("path") or "").strip("/"))
        if what:
            out.append(what); out.append("")
        hint = (n.get("attrs") or {}).get("how_to_read_this")
        if hint is not None:
            out.append(self._text_of(hint)); out.append("")
        out.append("%s   %d row%s%s" % (n["path"], len(table),
                                        "" if len(table) == 1 else "s",
                                        ("   run %d of %d" % (k, a.shape[0]))
                                        if per_run else ""))
        out.append("")
        out.append("  " + "  ".join(c.ljust(width[j])
                                    for j, c in enumerate(cols)))
        out.append("  " + "  ".join("-" * width[j] for j in range(len(cols))))
        for t in table:
            out.append("  " + "  ".join(t[j][:width[j]].ljust(width[j])
    ...
    def _draw_paired(self, n):
        """A named table, read down the columns rather than across.

        Names go on the left and the runs across the top, because the question
        people actually have is "did this setting differ between runs", and
        that is a glance down one line.
        """
        import numpy as np
        names = self._paired_names(n)
        a = np.asarray(n["obj"][...])
        rows, cols = a.shape
        t = self._text_widget()
        ...
        if what:
            out.append(what); out.append("")
        out.append("%s   %d rows (runs) x %d columns"
                   % (n["path"], rows, cols))
        out.append("")

        def cell(v):
            if a.dtype.kind in "SOU":
                s2 = self._text_of(v)
                return s2 if s2 != "" else "-"
            return _num(v)

        width = max([len(x) for x in names] + [10])
        width = min(width, 58)
        head = "  " + "name".ljust(width) + "".join(
            "%16s" % ("run %d" % i) for i in range(min(rows, 12)))
        out.append(head)
        out.append("  " + "-" * (len(head) - 2))
        same = 0
        for j, nm in enumerate(names):
            vals = [cell(a[i, j]) for i in range(rows)]
            if len(set(vals)) == 1:
                same += 1
            out.append("  " + nm[:width].ljust(width)
                       + "".join("%16s" % v for v in vals[:12]))
        if rows > 12:
            out.append("")
            out.append("  (showing the first 12 runs of %d)" % rows)
        out.append("")
        out.append("  %d of %d are the same in every run; the rest are where "
                   "the runs differ." % (same, len(names)))
        t.insert("1.0", "\n".join(out))
        t.configure(state="disabled")

    def _draw_table(self, n):
        import numpy as np
        t = self._text_widget()
        a = np.asarray(n["obj"][...])
        head = []
        ...
        if what:
            head.append(what)
        head.append("%s   %s" % (n["path"], self._dimlab(n["shape"])))
        cols = self._column_names(n, a)
        if a.ndim == 0:
            head += ["", "    %s" % _short_value(a)]
        elif a.ndim == 1:
            head.append("")
            for i, v in enumerate(a):
                head.append("  [%4d]  %s" % (i, _num(v)))
        else:
            head.append("")
            if cols:
                head.append("        " + "".join("%14s" % c for c in cols))
            for i in range(a.shape[0]):
                head.append("  [%4d]" % i
                            + "".join("%14s" % _num(v) for v in a[i]))
        t.insert("1.0", "\n".join(head))
        t.configure(state="disabled")

    def _column_names(self, n, a):
        """Name the columns of a table when the file says what they are. The
        conditions are the case that matters: a row of six numbers means
        nothing without them."""
        d = self.data
        if d["kind"] != "h5" or a.ndim != 2:
            return None
        p = n["path"].strip("/")
        try:
            if p.endswith("params") and "param_names" in d["h"].attrs:
                names = [self._text_of(v) for v in d["h"].attrs["param_names"]]
                if len(names) == a.shape[1]:
                    return names
                # A file whose names and columns disagree is worth saying so
                # about rather than quietly showing the wrong headings.
                return names[:a.shape[1]] + ["column %d" % i
                                             for i in range(len(names), a.shape[1])]
        except Exception:                                      # noqa: BLE001
            pass
        return None

    # --------------------------------------------------------------- picture
    def _draw_image(self, n):
        import numpy as np
        import matplotlib
        ...
        if self.var_view.get() != "Plane":
            return self._draw_3d(n)

        arr, xname, yname = self._slice(n)
        mask = self._mask_for(n)
        show = arr.copy()
        if mask is not None and mask.shape == show.shape:
            show = np.where(mask, show, np.nan)

        fin = show[np.isfinite(show)]
        vmin = float(fin.min()) if fin.size else 0.0
        vmax = float(fin.max()) if fin.size else 1.0
        # A field that is one value everywhere has no range to stretch a colour
        # bar over. Stretching one anyway draws a picture that looks like data.
        flat = (vmax <= vmin)
        if flat:
            vmax = vmin + 1e-9
        ...
        kw = dict(origin="lower", cmap=self.var_cmap.get(),
                  interpolation="nearest", aspect="equal")
        if self.var_log.get() and vmin > 0:
            im = ax.imshow(show, norm=LogNorm(vmin=vmin, vmax=vmax), **kw)
        else:
            im = ax.imshow(show, vmin=vmin, vmax=vmax, **kw)
        ax.set_xlabel(xname + ("   (flow enters left, leaves right)"
                               if xname == "x" else ""))
        ax.set_ylabel(yname)

        shape = n["shape"]
        held = self._held_labels(n["path"], shape,
                                 skip=(int(self.ax_x.get()), int(self.ax_y.get())))
        sub = ("   at " + ", ".join(held)) if held else ""
        rng = ("every value here is %.6g" % vmin) if flat \
            else ("range %.4g to %.4g" % (vmin, vmax))
        ax.set_title("%s%s\n%s" % (n["path"], sub, rng), fontsize=10)
        cb = fig.colorbar(im, ax=ax, fraction=0.036, pad=0.02)
        unit = self._unit_note(n)
        if unit:
        ...
        NavigationToolbar2Tk(canvas, self.plotarea).update()

    def _draw_3d(self, n):
        """The whole volume, in one of three ways, with nothing cut out.

        Which one to use is not a matter of taste:

          Solid    for a pore structure. The object IS its outer surface.
          Cloud    for a field. Low opacity per step, accumulated front to
                   back, so what is inside shows through what is in front.
          Surface  for a field, when you want a shape rather than a fog. The
                   surface where the field passes through one value, and the
                   level slider sweeps it through the volume.

        An outside-only rendering of a field, which is what this used to do,
        shows the skin of the pore space and nothing else. At forty percent
        porosity that skin is noise, and no amount of lighting makes it a
        picture of anything.
        """
        import numpy as np
        import matplotlib
        ...
        vol = self._volume(n)
        pore = self._volume_mask(n)
        is_structure = n["path"].rstrip("/").endswith(("material", "biofilm"))
        view = self.var_view.get()
        if is_structure and view in ("Cloud", "Surface"):
            view = "Solid"

        if is_structure:
            keep = (vol != 2)
            vals = vol[keep] if keep.any() else vol.ravel()
        else:
            keep = pore if pore is not None else np.isfinite(vol)
            vals = vol[keep & np.isfinite(vol)]
        # A single overshooting voxel sets the top of the colour scale and
        # pushes everything real into the bottom two percent of it, which is
        # how a field with a perfectly good gradient in it comes out as one
        # flat colour. Trimming the tails is on by default and the true range
        # is still printed, so nothing is hidden.
        if self.var_trim.get() and vals.size > 100:
            vmin = float(np.percentile(vals, 1.0))
            vmax = float(np.percentile(vals, 99.0))
            full = (float(vals.min()), float(vals.max()))
        else:
        ...
        elev, azim = float(self.var_elev.get()), float(self.var_azim.get())
        cmap = self.var_cmap.get()
        note = ""

        if view == "Surface":
            level = vmin + float(self.var_level.get()) * (vmax - vmin)
            img, hits = render_iso(vol, keep, level, vmin, vmax, cmap,
                                   elev=elev, azim=azim)
            if img is None:
                return self._blank(
                    "  Nothing sits at that value.\n\n  The field here runs "
                    "from %.4g to %.4g. Move the level slider into that range."
                    % (vmin, vmax))
            note = "the surface where it passes through %.4g, %s voxels" % (
                level, "{:,}".format(hits))
        elif view == "Cloud":
            img = render_volume(vol, keep, vmin, vmax, cmap, elev=elev,
                                azim=azim, mode="cloud",
                                opacity=float(self.var_opacity.get()))
            note = "seen through, near to far"
        else:
            img = render_volume(vol, keep, vmin, vmax, cmap, elev=elev,
                                azim=azim, mode="solid",
                                flat_colour=(0.72, 0.70, 0.67) if is_structure
                                else None)
            note = "the grains" if is_structure else "the nearest pore surface"

        if img is None:
            return self._blank("  There is nothing to draw here: every voxel "
                               "is either outside the pore space or not a "
                               "number.")
        ...
        shape = n["shape"]
        held = self._held_labels(n["path"], shape,
                                 skip=tuple(range(len(shape) - 3, len(shape))))
        sub = ("   at " + ", ".join(held)) if held else ""
        rng = "range %.4g to %.4g" % full
        if (vmin, vmax) != full:
            rng += "  (colours trimmed to %.4g - %.4g)" % (vmin, vmax)
        ax.set_title("%s%s\n%s,  %d x %d x %d,  %s"
                     % (n["path"], sub, note, shape[-3], shape[-2], shape[-1],
                        rng), fontsize=10)
        if not is_structure:
    ...
    def _unit_note(self, n):
        """Concentrations are stored divided by a scale. Say so on the bar
        rather than letting someone read a normalised number as a real one."""
        d = self.data
        if d["kind"] != "h5":
            return ""
        p = n["path"].strip("/")
        try:
            for pth, key, unit in (("samples/conc", "conc_scale", ""),
                                   ("samples/rate", "rate_scale", " per second")):
                if p != pth or key not in d["h"]["samples"].attrs:
                    continue
                sc = d["h"]["samples"].attrs[key]
                c = int(self.ax_pos[2].get()) if len(n["shape"]) >= 6 else 0
                if c < len(sc):
                    return ("stored value. Multiply by %.6g for the real one%s"
                            % (float(sc[c]), unit))
        except Exception:                                      # noqa: BLE001
            pass
        return ""
    ...
    def _adopt(self, data):
        ...
        for n in self.nodes.values():
            n["_fresh"] = True
        self.ax_x.set(0); self.ax_y.set(0); self.ax_pos = []
        self._axes_for = None
        self.head.configure(text="  " + data["title"])
        self._fill_tree()
        if not self.tree.selection():
            self._build_controls()
            self._blank("  " + data.get("summary", ""))
    ...
    def open_dataset(self):
        ...
        p = filedialog.askopenfilename(
            ...
            filetypes=[("HDF5 dataset", "*.h5 *.hdf5"), ("All files", "*.*")])
        if not p:
            return
        try:
            import h5py
            ...
            h = h5py.File(p, "r")
            self._h = h
            self._adopt(dict(kind="h5", h=h, path=p,
                             title=os.path.basename(p),
                             summary=_h5_summary(h, p)))
            self.app.status("Opened %s" % os.path.basename(p))
        except Exception as e:                                 # noqa: BLE001
            self.app.problem("Could not open that file: %s" % e)
    ...
    def open_geometry(self):
        ...
        try:
            ...
            for k in z.files:
                v = np.asarray(z[k])
                if v.ndim >= 1:
                    arrays[k] = v.astype("float32") if v.dtype.kind in "biu" else v
            if not arrays:
                raise ValueError("nothing with a shape in that file")
            mat = arrays.get("material")
            por = ("porosity %.3f" % float((np.asarray(z["material"]) == 2).mean())
                   if "material" in z else "")
            self._adopt(dict(kind="np", path=p, title=os.path.basename(p),
                             arrays=arrays, mat=mat,
                             summary="A pore structure.  %s  %s"
                                     % (" x ".join(str(s) for s in
                                                   list(arrays.values())[0].shape),
                                        por)))
            self.app.status("Opened geometry %s" % os.path.basename(p))
        except Exception as e:                                 # noqa: BLE001
    ...
    def open_2d(self):
        ...
        try:
            ...
            g = to_complab(_reshape_flat(a))
            vol = g.astype("float32")[:, :, None]
            self._adopt(dict(kind="np", path=p, title=os.path.basename(p),
                             arrays={"pore space": vol}, mat=vol,
                             summary="A 2D domain.  %s   porosity %.3f   "
                                     "(index order auto-detected)"
                                     % (" x ".join(map(str, g.shape)),
                                        float((g == 2).mean()))))
            self.app.status("Opened 2D domain %s" % os.path.basename(p))
        except Exception as e:                                 # noqa: BLE001
    ...
    def open_pred(self):
        ...
        try:
            ...
            if not arrays:
                raise ValueError("nothing with a shape in that file")
            first = list(arrays.values())[0]
            self._adopt(dict(kind="np", path=p, title=os.path.basename(p),
                             arrays=arrays, mat=None,
                             summary="%d array%s.  first one %s"
                                     % (len(arrays), "" if len(arrays) == 1 else "s",
                                        " x ".join(map(str, first.shape)))))
            self.app.status("Opened prediction %s" % os.path.basename(p))
        except Exception as e:                                 # noqa: BLE001
            self.app.problem("Could not open that prediction: %s" % e)

    def save_png(self):
        if self.fig is None:
            messagebox.showinfo(APP, "There is no picture on screen to save yet.")
            return
...
def _human_bytes(n):
    ...
    for u in ("bytes", "KB", "MB", "GB"):
        if n < 1024 or u == "GB":
            return "%.1f %s" % (n, u) if u != "bytes" else "%d bytes" % n
        n /= 1024.0
    return "%.1f GB" % n
...
def _short_value(v, width=70):
    s = _long_value(v).replace("\n", " ")
...
def _long_value(v):
    ...
    if isinstance(v, np.ndarray):
        if v.dtype.kind in "SOU":
            return "  ".join(Viewer._text_of(x) for x in v.reshape(-1))
        return "  ".join(_num(x) for x in v.reshape(-1)[:200])
    return str(v)


def _h5_summary(h, path):
    """One paragraph about a file, written from what is actually in it rather
    than from what the layout says should be. A file missing a piece says so."""
    import numpy as np
    bits = []
    a = h.attrs
    ...
    def txt(k):
        ...
        return [_long_value(v)]

    sp = txt("species")
    shape = a.get("shape")
    if shape is not None:
        shape = [int(x) for x in np.asarray(shape).reshape(-1)]
        dim = 2 if (len(shape) >= 3 and shape[2] == 1) else 3
        bits.append("A %dD dataset on a %s grid." % (dim, " x ".join(map(str, shape))))
    try:
        ns = int(h["samples/conc"].shape[0])
        nt = int(h["samples/conc"].shape[1])
        bits.append("%d run%s, %d snapshot%s each."
                    % (ns, "" if ns == 1 else "s", nt, "" if nt == 1 else "s"))
    except Exception:                                          # noqa: BLE001
        pass
    try:
        bits.append("%d rock%s." % (len(h["geom/gid"]),
                                    "" if len(h["geom/gid"]) == 1 else "s"))
    except Exception:                                          # noqa: BLE001
        pass
    if sp:
        bits.append("Species: " + ", ".join(sp) + ".")

    missing = []
    for k, why in (("inputs", "the input files each run was given"),
                   ("samples/rate", "reaction rates"),
                   ("samples/dconc_dt", "rates of change")):
        if k not in h:
            missing.append(why)
    if missing:
        last = missing.pop()
        phrase = (", ".join(missing) + " and " + last) if missing else last
        bits.append("Not in this one: " + phrase + ".")
    bits.append("")
    bits.append(path)
    return "  ".join(bits[:-2]) + "\n\n" + path
...
ONE CHEMICAL PER MODEL. The network has a single output, exactly as the
published 2D model does, so a dataset holding Ac, A, P and Bio needs four
training runs. The "Which chemical to predict" box on the Train page chooses
which one; left blank, the first chemical in the dataset is used and the run
...
class Studio(tk.Tk):
    ...
    def _menu(self):
        ...
        f.add_command(label="Load a settings file into this page...",
                      command=lambda: self.current.load_settings_file()
                      if isinstance(self.current, ActionPage) else
                      messagebox.showinfo(
                          APP, "Open a page first.\n\nIn the pipeline on the "
                               "left, under Simulate, choose 'Run 3D "
                               "simulations to train on'. The settings-file box "
                               "is the second one down on that page, and this "
                               "menu item fills it in."))
        f.add_separator()
    ...
    def build_tree(self):
        ...
        if self.mode.get() == "3D":
            groups = [("Set up", ["install", "checkall",
                                  "dataset3d", "geometry"]),
                      ("Simulate", ["complab_campaign", "collect_complab_output",
                                    "collect_foreign", "flow_features", "ingest2d"]),
                      ("Learn", ["train", "evaluate", "sweep"]),
                      ("The flow field", ["train_velocity", "predict_velocity"]),
                      ("Predict", ["predict"])]
            for g, keys in groups:
                node = self.tree.insert("", "end", text=" " + g, open=True)
                for k in keys:
                    self.tree.insert(node, "end", iid="act:" + k, text=label(k))
            # AUDIT GUI-02. The five-step flow pipeline, which is where the
            # velocity capability lives now that it is no longer a box on the
            # training page.
            flow = self.tree.insert("", "end", text=" The flow pipeline",
                                    open=True)
            self.tree.insert(flow, "end", iid="flowpipe",
                             text="   Run the five steps in order")
            ...
            groups = [("Set up", ["install", "checkall",
                      ...
                      ("Simulate", ["collect_complab_output", "collect_foreign",
                                    "flow_features"]),
                      ("Learn", ["train", "evaluate", "sweep"]),
                      ("The flow field", ["train_velocity", "predict_velocity"]),
                      ("Predict", ["predict"])]
    ...
    def open_flow_pipeline(self):
        """Show the five-step flow panel, building it the first time.

        AUDIT GUI-02. It is a page like any other; it just drives five actions
        instead of one.
        """
        if getattr(self, "_flow_page", None) is None:
            self._flow_page = FlowPipelinePage(self.pages, self)
        for w in self.pages.winfo_children():
            w.pack_forget()
    ...
    def update_monitor(self, final=False, secs=None):
        ...
        try:
            ...
            # One figure per epoch update, never closed, is three hundred
            # figures by the end of a training run.
            old = getattr(self, "_mon_fig", None)
            if old is not None:
                try:
                    plt.close(old)
                except Exception:                              # noqa: BLE001
