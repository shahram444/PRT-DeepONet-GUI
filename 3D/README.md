...
Everything that is *not* CompLaB. The solver lives in `CompLB3D_ready/`.

```
geometries/       140 ready-made 64^3 pore geometries, Jung's morphology
tools/            generate geometries, run a complab_campaign, collect_complab_output one training file
model/            the 3D PRT-DeepONet: architecture, training, inference, evaluation
                    ...
                    evaluate.py   held-out accuracy + the paper's figures
                    make_figures.py        the 2D and 3D flow / biotic / abiotic rendering
example_outputs/  what the figures look like, before you have run anything
```
...
140 directories, each holding what CompLaB reads and what the model reads:

```
geom_0000/
    geometry.dat            <- CompLaB reads this (copy to CompLB3D/input/)
    geometry.dims
    geom_0000.npz           <- material, material_jung, gdf, edt, meta
    geom_0000_slices.png    <- 2D slices, geodesic field, tortuosity excess
    geom_0000_3d.png        <- 3D render (every 10th geometry)
manifest.csv                <- porosity, correlation length, tortuosity for all 140
```
...
PRT-DeepONet release. What they actually are:
...
**Porosity 0.486 / 0.495 / 0.524 / 0.609** across the four domains I sampled, so
the 2000-domain training set spans roughly 0.45–0.65.
...
python collect_complab_output.py --complab_campaign ./complab_campaign --geometries ../geometries \
...
`--batches 5` splits the complab_campaign into five independent sub-campaigns of 560 runs
...
`complab_campaign.py status` and `retry` walk every batch, and `collect_complab_output.py --complab_campaign`
accepts the complab_campaign root, several paths, or one batch alone.
...
different Péclet number. For a single manual run use `CompLB3D_ready/CompLaB.xml`.
...
worse, puts different settings_and_units times in the same slot. `--n-times K` resamples
...
float16; all five batches at K=6 is about 35 GB, so collect_complab_output per batch unless the
...
```bash
python model/train.py --data tools/dataset/dataset_reader.h5 --out runs/gdf
...
python model/evaluate.py --data dataset/dataset_reader.h5 --out figs/ablation \
...
Worked examples of all of this are in `example_outputs/`, with a README that
says exactly which parts of them are real physics and which are placeholder
numbers from an untrained checkpoint.
...
tr, te = split_by_geometry("dataset/dataset_reader.h5", frac=0.15)
train = PRT3DDataset("dataset/dataset_reader.h5", indices=tr, n_points=8192)
