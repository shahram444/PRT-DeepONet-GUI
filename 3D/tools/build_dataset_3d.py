...
def geometry_at_porosity(shape, phi_want, seed, tol=0.004, tries=14):
    """A structure whose FINAL porosity is the one that was asked for.

    geometry() thresholds the random field so that a fraction phi of it is
    pore, and then does three things that change that fraction: it opens the
    first and last three slices completely, it seals the four sides, and it
    fills in whatever the inlet cannot reach. The inlet and outlet buffers
    alone are six slices, which on a 32-voxel domain is nearly a fifth of the
    volume, so asking for 0.40 gave back 0.44. Anyone who then measured the
    porosity of the file would find a number nobody asked for, and a paper
    quoting the requested range would be quoting the wrong range.

    So the knob is turned until the answer is right. What comes out rises
    monotonically with what goes in, so bisection finds it, and each step is a
    threshold and one connected-component pass -- microseconds against the
    Stokes solve that follows. The same seed is used throughout, so it is the
    same random field being re-cut, not a different structure each time.

    Returns (geometry, achieved porosity). If even a fully open field cannot
    reach the target -- which happens when the sides and the connectivity
    filter take more than the shortfall -- the closest attempt is returned
    rather than an error, and the caller reports what it actually got.
    """
    lo, hi = 0.02, 0.995
    best = None
    for _ in range(tries):
        mid = 0.5 * (lo + hi)
        g = geometry(shape, mid, seed=seed)
        got = float((g == PORE).mean())
        if best is None or abs(got - phi_want) < abs(best[1] - phi_want):
            best = (g, got)
        if abs(got - phi_want) <= tol:
            return g, got
        if got < phi_want:
            lo = mid
        else:
...
def _values(cli_list, cfg_list, name):
    """The exact numbers the user asked for, or None if they gave a range.

    A list on the command line wins over a list in the settings file, so one
    run can be redirected without editing the file that describes it. Anything
    that is not a positive number is an error here rather than a puzzle later:
    a Peclet number of zero means no flow, which is not a sweep point, and a
    negative Damkohler number means the reaction runs backwards.
    """
    vals = cli_list if cli_list else cfg_list
    if not vals:
        return None
    out = [float(v) for v in vals]
    bad = [v for v in out if not np.isfinite(v) or v <= 0]
    if bad:
        raise SystemExit(
            "%s was given %s. Every value has to be a positive number: these "
            "are ratios, and a zero or a negative one does not describe an "
            "experiment." % (name, ", ".join(repr(b) for b in bad)))
    return out


def _condition_grid(pe_v, da_v, da2_v, pe_lo, pe_hi, da_lo, da_hi,
                    da2_lo, da2_hi, biotic, abiotic, n_sets, rng):
    """The list of (Peclet, Damkohler, abiotic Damkohler) each structure gets.

    Where the user gave exact values, every combination of them is built, so
    three Peclet numbers and two Damkohler numbers is six conditions. Where
    they gave a range instead, that entry is drawn log-uniformly, freshly for
    every condition, so the runs still spread over the range rather than
    repeating one draw.

    Returns None when nothing was listed, which means the caller keeps its
    original behaviour and draws all three per run.
    """
    if not (pe_v or da_v or da2_v):
        return None
    pe_s = pe_v or [None]
    da_s = (da_v or [None]) if biotic else [0.0]
    da2_s = (da2_v or [None]) if abiotic else [0.0]
    combos = [(p, d, d2) for p in pe_s for d in da_s for d2 in da2_s]
    if n_sets is not None and n_sets != len(combos):
        # The user asked for a different number of runs per structure than the
        # combinations make. Fewer means the first n; more means going round
        # again. Either is a deliberate thing to want, so it is allowed, but it
        # is said out loud because it is also an easy thing to do by accident.
        print("  note: %d combination(s) of the values you listed, but %d run(s)"
              % (len(combos), n_sets))
        print("        per structure were asked for, so the list is %s."
              % ("truncated" if n_sets < len(combos) else "repeated"))
        combos = [combos[i % len(combos)] for i in range(n_sets)]
    return combos


def _sweep_line(vals, lo, hi, unit=""):
    """How a swept quantity is described in the report: the exact numbers if
    they were listed, otherwise the range they are drawn from."""
    if vals:
        return "exactly %s%s" % (", ".join("%g" % v for v in vals), unit)
    return "swept %g to %g%s" % (lo, hi, unit)


def announce(cfg, n_geom, n_sets, shape, pe_lo, pe_hi, da_lo, da_hi,
             da2_lo, da2_hi, pe_vals=None, da_vals=None, da2_vals=None,
             phi_vals=None, phi_lo=0.30, phi_hi=0.50):
    ...
    print("  porosity                 %s"
          % _sweep_line(phi_vals, phi_lo, phi_hi))
    if phi_vals:
        print("                           structures cycle through that list")
        print("                           in order.")
    if by_pressure:
        ...
        print("  flow driven by           the PECLET NUMBER, %s."
              % _sweep_line(pe_vals, pe_lo, pe_hi))
    ...
    print("  biotic reaction          %s"
          % ("ON, Damkohler %s" % _sweep_line(da_vals, da_lo, da_hi)
             if cfg.biotic_enabled else "OFF"))
    print("  abiotic reaction         %s"
          % ("ON, Damkohler %s%s"
             % (_sweep_line(da2_vals, da2_lo, da2_hi),
...
def main():
    ...
    ap.add_argument("--pe-values", type=float, nargs="+", default=None,
                    metavar="PE",
                    help="the exact Peclet numbers to use, instead of a range: "
                         "--pe-values 1 10 50")
    ap.add_argument("--da-values", type=float, nargs="+", default=None,
                    metavar="DA",
                    help="the exact biotic Damkohler numbers to use")
    ap.add_argument("--da-abio-values", type=float, nargs="+", default=None,
                    metavar="DA",
                    help="the exact abiotic Damkohler numbers to use")
    ap.add_argument("--phi-values", type=float, nargs="+", default=None,
                    metavar="PHI",
                    help="the exact porosities to build structures at, instead "
                         "of a range: --phi-values 0.35 0.40 0.45. Structures "
                         "cycle through the list in order")
    ap.add_argument("--phi-min", type=float, default=None,
    ...
    phi_lo, phi_hi = _range(a.phi_min, a.phi_max, cfg.porosity_min,
                           cfg.porosity_max, 0.30, 0.50)
    # THE EXACT NUMBERS, if any were asked for. A list beats a range, and a
    # list on the command line beats a list in the settings file.
    pe_vals = _values(a.pe_values, cfg.peclet_values, "Peclet")
    da_vals = _values(a.da_values, cfg.damkohler_values,
                      "the biotic Damkohler number")
    da2_vals = _values(a.da_abio_values, cfg.damkohler_abiotic_values,
                       "the abiotic Damkohler number")
    phi_vals = _values(a.phi_values, cfg.porosity_values, "porosity")
    if phi_vals:
        bad = [v for v in phi_vals if not 0.0 < v < 1.0]
        if bad:
            raise SystemExit(
                "porosity was given %s. Porosity is a fraction of the volume, "
                "so every value has to lie between 0 and 1."
                % ", ".join(repr(b) for b in bad))
        low = [v for v in phi_vals if v < 0.20]
        if low:
            print("  note: porosity %s is at or below the three-dimensional "
                  "percolation threshold of about 0.20."
                  % ", ".join("%.3g" % v for v in low))
            print("        Structures at that porosity mostly do not connect "
                  "inlet to outlet, and the")
            print("        generator will spend a long time rejecting them "
                  "before it gives up.")
    by_pressure = str(cfg.flow_driver).lower().startswith("pres")
    ...
    combos = _condition_grid(pe_vals, da_vals, da2_vals, pe_lo, pe_hi,
                             da_lo, da_hi, da2_lo, da2_hi,
                             cfg.biotic_enabled, cfg.abiotic_enabled,
                             a.n_sets, rng)
    # Runs per structure: what was asked for, or -- when exact conditions were
    # listed -- one run for every combination of them, which is what a person
    # who writes out three Peclet numbers and two Damkohler numbers means.
    if a.n_sets is None:
        a.n_sets = len(combos) if combos else 5
    n_sets = a.n_sets

    announce(cfg, a.n_geom, a.n_sets, shape0, pe_lo, pe_hi, da_lo, da_hi,
             da2_lo, da2_hi, pe_vals, da_vals, da2_vals, phi_vals,
             phi_lo, phi_hi)
    ...
    print()
    # --show-settings stops HERE rather than earlier, so what it shows you is
    # the whole plan -- the conditions and the porosities as well as the
    # physics -- and not just the half of it that comes from the settings file.
    if a.show_settings:
        print("--show-settings was given, so nothing was simulated. Remove it "
              "to run this.")
        return
    ...
    while made < G:
        ...
        if tries > 20 * G:
            raise SystemExit("could not find %d percolating structures; try a "
                             "higher --phi-min" % G)
        # POROSITY. Either drawn from the range, or taken from the list the
        # user wrote, cycling through it so ten structures over a list of two
        # porosities gives five of each. It is indexed by how many structures
        # have been ACCEPTED, not by how many were attempted, so a rejected
        # non-percolating structure does not shift the whole list along.
        phi = (float(phi_vals[made % len(phi_vals)]) if phi_vals
               else float(rng.uniform(phi_lo, phi_hi)))
        phi_want = phi
        g, phi_got = geometry_at_porosity(shape, phi_want,
        ...
        if not by_pressure:
            print("     to reach Peclet %g to %g on this one the pressure "
                  ...
                  flush=True)
        _phi_now = float((g == PORE).mean())
        _miss = ("" if abs(_phi_now - phi_want) <= 0.006
                 else "  (asked for %.3f; this is as close as the pore space "
                      "gets while still spanning the sample)" % phi_want)
        print("  structure %3d  phi=%.3f  gdf_max=%5.1f  umean=%.2e%s"
              % (made, _phi_now, gdf[made].max(),
                 float(sp[g == PORE].mean()), _miss), flush=True)
        made += 1
    ...
    for i in range(G):
        for j in range(a.n_sets):
            want = combos[j] if combos else (None, None, None)
            ...
            if not cfg.biotic_enabled:
                da = 0.0
            elif want[1] is not None:
                da = float(want[1])
            else:
                da = float(10 ** rng.uniform(np.log10(da_lo), np.log10(da_hi)))
            if not cfg.abiotic_enabled:
                da2 = 0.0
            elif want[2] is not None:
                da2 = float(want[2])
            else:
